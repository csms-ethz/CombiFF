...
const Atom& AtomType::GetElementType() const {
  return element_type;
}

const std::string& AtomType::GetAtomTypeName() const {
  return atom_type_name;
}

const std::string& AtomType::GetVersion() const {
  return version;
}
...
AtomTypeSet::AtomTypeSet(const std::string& version,
                         ...
                         size_t index)
  : version(version), atom_type_set_name(set_name), contained_atoms(0), index(index) {}
...
const AtomSet& AtomTypeSet::GetAtomSet() const {
  return contained_atoms;
}

const std::string& AtomTypeSet::GetName() const {
  return atom_type_set_name;
}

const std::string& AtomTypeSet::GetVersion() const {
  return version;
}

size_t AtomTypeSet::GetIndex() const {
  return index;
}
...
void AtomTypeSet::SetVersion(const std::string& v) {
  version = v;
}

void AtomTypeSet::AddAtom(const Atom& atom) {
  contained_atoms.emplace(atom);
}
  ...
  for (auto && file_name : atom_types_file_names) {
    ...
    for (auto && child : root.children) {
      if (child->tag == "simpleatomtypes") {
        ...
        throw unexpected_tag_error(child->tag, "simpleatomtypes or atomtypesets");
    }
  }
}

void AddSimpleAtomTypes(AtomTypeSetMap& atom_type_set, const XmlElement_ptr simpleatomtypes,
  ...
  for (auto && atomtype : simpleatomtypes->children) {
    ...
    if (id == "any")
      throw input_error("atom type Set 'any' is reserved keyword, cannot be explicitly listed in input\n");

    if (std::isdigit(atom_name.back()) && atom_name.size() > 2 && atom_name[atom_name.size() - 2] == 'H') {
    ...
    if (num_H.size())
      atom.SetNumFixedHydrogens(std::stoi(num_H));
...
void AddAtomTypeSets(AtomTypeSetMap& atom_type_set, const XmlElement_ptr atomtype_set_ptrs,
                     const std::string& version, size_t& idx) {
  for (auto && atomtype_set_ptr : atomtype_set_ptrs->children) {
    ...
    if (atom_type_set.find(set_name) != atom_type_set.end())
      throw input_error("listed atom type name or atom type Set name " + set_name + " more than once\n");
    ...
    for (auto && member : atomtype_set_ptr->children) {
      ...
      if (it != atom_type_set.end()) {
        if (it->second.GetVersion() != version)
          std::cout << "?Warning: atom type (Set) " << member_id << " has version " << it->second.GetVersion()
        ...
        throw input_error(member_id + " is neither an att nor an ats (or not defined before the current ats "
                          + set_name + "\n");
    }
  }
...
void CheckAtomTypeXML(const XmlElement_ptr atomtype) {
  ...
  if (!(id.front() >= 'A' && id.front() <= 'Z'))
    throw input_error("Please start atom type ids with upper case letter in <atomtype> tag. Not the case for atomtype "
                      + id + '\n');
}
...
void CheckAtomTypeSetXML(const XmlElement_ptr atomtype_set_ptr) {
  ...
  if (!(name.front() >= 'A' && name.front() <= 'Z'))
    throw input_error("Please start atom type Set names with upper case letter in <atomtypeset> tag. Not the case for atomtype Set "
                      + name + '\n');
...
std::ostream& operator<<(std::ostream& stream, const AtomTypeSet& type_set) {
  ...
  for (auto && tt : type_set.GetAtomTypeNames())
    stream << tt << " ";

...
//if new PropertyType is added, also adapt the vector of properties used to iterate in mtb.h
typedef enum {bond, angle, improper_dihedral, torsional_dihedral, num_topological_property_types} PropertyType;
...
class TopologicalPropertyBase {
 ...
 public:
  ...
  TopologicalPropertyBase(const std::string& property_name, const size_t num_involved_atoms) ;
  ...
  void SetAttributes(const std::unordered_map<std::string, std::string>& attrib) ;
...
static const std::unordered_map  <TopologicalPropertyTypeID, PropertyType> topologicalPropertyMap{
  {"bond", bond},
  {"angle",  angle},

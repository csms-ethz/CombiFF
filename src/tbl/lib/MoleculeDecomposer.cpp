...
/*
!!!!!!!!!!!!!!!!!!!!HalfLink.fragIdx is one higher than the C++ index of the fragment in the fragments vector!!!!!!!!
i.e. for frag[0] the fragIdx is one!!!!
*/

FamilyDecomposer::FamilyDecomposer(const std::string& filename_family_isomer_enumeration,
...
void FamilyDecomposer::CreateFamilyMoleculeDecompositions() {
  XmlParserIn family_isomer_enumeration_parser(filename_family_isomer_enumeration,
                                               XmlParserIn::read_element_by_element);
  family_isomer_enumeration_parser.ReadUntilElement("enumeration_time");
  auto&& family_isomer_enumeration_root = family_isomer_enumeration_parser.GetRoot();
  CheckXMLFormat(family_isomer_enumeration_root);
  filename_molecule_decomposition = (io_file_properties.output_dir_molecule_decompositions.size() ?
                                     io_file_properties.output_dir_molecule_decompositions
                                     : io_file_properties.output_dir) + "molecule_decompositions_" + family_code + ".xml";
  ...
  molecule_decompositions_root->attributes["tbl_version"] = combi_ff::current_version;
  molecule_decompositions_root->SetAttribute(family_isomer_enumeration_root, "family_code");
  molecule_decompositions_root->SetAttribute(family_isomer_enumeration_root, "family_version");
  molecule_decompositions_root->SetAttribute(family_isomer_enumeration_root, "enu_version");
  ...
  if (num_isomers) {
    ...
    while (family_isomer_enumeration_parser.ReadUntilElement("isomer_list")) {
      ...
      for (auto && isomer_list : family_isomer_enumeration_parser.GetCurNode()->children) {
        for (auto && isomer : isomer_list->children) {
          ...
          XmlElement& molecule_decomposition = *(molecule_decompositions_root->GetLastChild());
...
void FamilyDecomposer::CheckXMLFormat(XmlElement& family_isomer_enumeration_root) {
...
size_t FamilyDecomposer::GetNumIsomers(const XmlElement& family_isomer_enumeration_root) {
...
MoleculeDecomposer::MoleculeDecomposer(const std::string& smiles,
                                       const std::vector<TblFragment>& tbl_fragments, XmlElement& molecule_decomposition)
...
void MoleculeDecomposer::CreateMoleculeDecomposition(XmlElement& molecule_decomposition) {
  ...
  for (size_t i = 0; i < links.size(); i++) {
    for (size_t j = i + 1; j < links.size(); j++) {
      if (links[i].GetHalfLink1()->GetFragmentIndex() > links[j].GetHalfLink1()->GetFragmentIndex())
        std::swap(links[i], links[j]);

      else if (links[i].GetHalfLink1()->GetFragmentIndex() == links[j].GetHalfLink1()->GetFragmentIndex()
               && links[i].GetHalfLink2()->GetFragmentIndex() > links[j].GetHalfLink2()->GetFragmentIndex())
        std::swap(links[i], links[j]);
    }
  }

  for (auto && frg : fragments)
    std::cout << frg->GetCode() << " ";
  ...
  for (auto && frag : fragments) {
    ...
    fragments_element.GetLastChild()->attributes["id"] = "f" + std::to_string(++i);
  ...
  for (auto && link : links) {
    ...
    involved_fragment_element1.GetLastChild()->value = "f" + std::to_string(
                                                         link.GetHalfLink1()->GetFragmentIndex());
    involved_fragment_element1.AddElement("linksite");
    involved_fragment_element1.GetLastChild()->value = link.GetHalfLink1()->GetLinkatomName();
    linkage_element.AddElement("involved_fragment");
    auto&& involved_fragment_element2 = *(linkage_element.GetLastChild());
    involved_fragment_element2.AddElement("fragment_id");
    involved_fragment_element2.GetLastChild()->value = "f" + std::to_string(
                                                         link.GetHalfLink2()->GetFragmentIndex());
    involved_fragment_element2.AddElement("linksite");
    involved_fragment_element2.GetLastChild()->value = link.GetHalfLink2()->GetLinkatomName();
  }
}
...
bool MoleculeDecomposer::FindBondLinkingMatch(const TblFragment& frag) {
  ...
  std::vector<const std::unordered_set<std::string>*> fragment_atom_names_sets(0);
  ...
  for (size_t i = 0; i < n; i ++) {
    ...
    if (fragment_tbl_atoms[i].IsCoreAtom()) {
      ...
      for (size_t j = 0; j < m; j++) {
        if (fragment_atom_set[i]->find(atoms[j].GetElementSymbol()) != fragment_atom_set[i]->end()) {
          if (!(is_assigned_core_atom)[j]) {
            if (!is_assigned_link_atom[j]) {
              ...
              for (auto && linkatom : link_atoms[j]) {
                if (linkatom->GetAtomTypes().GetAtomTypeNames().find(fragment_tbl_atoms[i].GetAtomTypes().GetName())
      ...
      for (size_t j = 0; j < m; j++) {
        if (fragment_atom_set[i]->find(atoms[j].GetElementSymbol()) != fragment_atom_set[i]->end()
            || fragment_atom_set[i]->find(atoms[j].GetUnitedAtomSymbol()) != fragment_atom_set[i]->end()) {
          if (!is_assigned_core_atom[j] ||
              fragment_atom_names_sets[i]->find(core_atoms[j]->GetAtomTypes().GetName()) !=
    ...
    if (!potential_match) {
      //std::cout << "found no match for " << fragment_tbl_atoms[i].GetAtomTypes() << std::endl;
      return false;
    }
  }
  ...
  if (match) {
    ...
    if (unlinked) {
      ...
      //i are the indices of the atoms in the fragment
      for (size_t i = 0; i < n; i++) {
        //j is the index of the atom in the adjacency matrix that is matched to atom i in the fragment
        j = GetMatchIndex(M, i);

        //if atom i in the fragment is a core atom, simply assign it as a core atom
        if (fragment_tbl_atoms[i].IsCoreAtom()) {
          //std::cout << "  case core\n";
          ...
          //check if atom j in the adjacency matrix is already an assigned core atom
          if ((is_assigned_core_atom)[j]) {
            //ii is the index of the core atom in the fragment that is connected to the link atom
            size_t ii = frag.GetCoreIndex(i);
            //k is the index of the atom in the adjacency matrix that is matched to the core atom ii in the fragment
            size_t k = GetMatchIndex(M, ii);
            //core is the index of the atom in the adjacency matrix that is matched to link atom i in the fragment
            size_t core = GetMatchIndex(M, i);

            //search in the existing half_links for the matching halflink
            for (auto && halflink = half_links.begin(); halflink != half_links.end(); halflink++) {
              if (halflink->GetAtomIndex() == k && halflink->GetCoreIndex() == core) {
                std::string name_link_atom_2 = fragment_tbl_atoms[i].GetAtomID();
                std::string name_link_atom_1 = halflink->GetLinkatomName();
                links.push_back(Link(HalfLink(halflink->GetAtomIndex(), halflink->GetCoreIndex(),
            ...
            half_links.push_back(HalfLink(j, GetMatchIndex(M, frag.GetCoreIndex(i)),
                                          fragments.size(),
...
bool MoleculeDecomposer::CheckValidity(XmlElement& molecule_decomposition) {
  if (!A.hasCycle() && fragments.size() != links.size() + 1) {
    molecule_decomposition.value = "\n" + std::string(molecule_decomposition.distance * 2 + 2, ' ')
                                   + "<!-- unable to find a valid decomposition for this molecule with the given fragments because\n"
                                   + std::string(molecule_decomposition.distance * 2 + 2, ' ')
                                   + " number of fragments (" + std::to_string(fragments.size()) +
                                   ") is not equal to number of links plus one "
                                   + "(" + std::to_string(links.size()) + " + 1 = " + std::to_string(links.size() + 1) + ")\n"
                                   + std::string(molecule_decomposition.distance * 2 + 2, ' ')
                                   + "fragments are:\n"
                                   + std::string(molecule_decomposition.distance * 2 + 2, ' ');
    ...
    for (auto && link : links)
      molecule_decomposition.value += fragments[link.GetHalfLink1()->GetFragmentIndex() - 1]->GetCode() +
                                      "[" +
  ...
  for (size_t i = 0; i < A.GetN(); i++) {
    if (!(is_assigned_core_atom[i] || is_assigned_link_atom[i] ||
      ...
      molecule_decomposition.value = "\n" + std::string(molecule_decomposition.distance * 2 + 2, ' ')
                                     + "<!-- unable to find a valid decomposition for this molecule with the given fragments because\n"
                                     + std::string(molecule_decomposition.distance * 2 + 2, ' ')
                                     + " atom " + std::to_string(i) + " (" + A.GetAtomVector()[i].GetUnitedAtomSymbol()
...
size_t MoleculeDecomposer::GetMatchIndex(const ComparisonMatrix& M, const size_t i) {
  for (size_t j = 0 ; j < M.GetM(); j++) {
    if (M.GetElement(i, j))
      return j;
  }

  throw std::runtime_error("couldn't find match index for the above ComparisonMatrix for i=" +
                           std::to_string(i) + "\n");
}

bool MoleculeDecomposer::UllmannMatch(ComparisonMatrix& M, std::vector<bool>& matched_columns,
                                      std::vector<bool>& matched_rows,
                                      int k, const size_t n, const size_t m, const FragmentMatrixTbl& fragment_matrix,
  ...
  for (size_t l = 0; l < m; l++) {
    if (M.GetElement(k + 1, l) == true && (matched_columns)[l] == false &&
      ...
      if (Refine(M, k + 1, n, m, fragment_matrix, fragment_tbl_atoms)) {
        if (UllmannMatch(M, matched_columns, matched_rows, k + 1, n, m, fragment_matrix,
                         fragment_tbl_atoms))
          return true;
      }
...
bool MoleculeDecomposer::Refine(combi_ff::ComparisonMatrix& M, int k, const size_t n,

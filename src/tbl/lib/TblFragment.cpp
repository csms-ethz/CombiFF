...
TblFragment::TblFragment(const std::string& version, const std::string& code, size_t N)
  : version(version),
    F(FragmentMatrixTbl(N)),
    code(code),
    atom_types(N) {}

TblFragment::TblFragment(const std::string& version,
                       const std::string& code,
                       size_t N,
                       const std::vector<TblAtom>& atom_types,
                       const std::vector<std::shared_ptr<TopologicalPropertyBase>>& topological_properties,
  ...
  : version(version),
    ...
    n_core_atoms(0),
    cyclic(false) {
  //calculate the number of single, double, triple and quadruple bonds in the Fragment
  for (size_t i = 0; i < N; i++) {
    for (size_t j = i + 1; j < N; j++) {
      if (F.GetElement(i, j) == 1)
        ...
        throw input_error("fragment has unrecognized bond degree " + std::to_string(F.GetElement(i,
                                                                                                 j)) + '\n');
    }
  }
...
GetTER METHODS
*************/
std::string TblFragment::GetCode() const {
  return code;
}
...
const FragmentMatrixTbl& TblFragment::GetMatrix() const {
  return F;
}

size_t TblFragment::GetNS() const {
  return n_single;
}

size_t TblFragment::GetNA() const {
  return n_aromatic;
}

size_t TblFragment::GetND() const {
  return n_double;
}

size_t TblFragment::GetNT() const {
  return n_triple;
}

size_t TblFragment::GetNQ() const {
  return n_quadruple;
}

size_t TblFragment::GetNumCoreAtoms() const {
  return n_core_atoms;
}

bool TblFragment::HasCycle() const {
  return cyclic;
}

const std::vector<std::shared_ptr<TopologicalPropertyBase>>& TblFragment::GetTopologicalProperties()
const {
  return topological_properties;
}
const std::string& TblFragment::GetVersion() const {
  return version;
}

//Get the core index that belongs to the atom with the index idx. If the atom itself is
//a core atom, idx is returned, and otherwise, the connections of atom with index idx are
//searched until a core atom is found
size_t TblFragment::GetCoreIndex(const size_t idx) const {
  if (atom_types[idx].GetLinkageType() == core) return idx;

  else {
    for (size_t i = 0; i < F.GetN(); i++) {
      if (F.GetElement(i, idx) && atom_types[i].IsCoreAtom())
        return i;
    }
  }

  throw std::runtime_error("atom " + std::to_string(idx) + " in fragment " + code +
                           " is not connected to a core atom\n");
}
...
void TblFragment::DetermineCoreAtoms() {
  ...
  for (auto && atom : atom_types) {
    if (atom.GetLinkageType() == core)
      ...
      if (!found)
        throw input_error("link atom " + atom.GetAtomID() + " is not connected to a core atom in fragment "
                          + code);
    }
  }
}
...
/********************************************************************************
CREATE THE TblFragment GIVEN IN THE INPUT FILE fragFile AND SAVE THEM TO tbl_fragments
********************************************************************************/
void CreateTblFragments(const std::list<std::string>& frag_file_names,
                       std::vector<TblFragment>& tbl_fragments,
                       const std::vector<std::string>& fragments_to_use, const AtomTypeSetMap& atom_type_set) {
  ...
  for (auto && fragFileName : frag_file_names) {
    ...
    if (version != combi_ff::current_version)
      std::cout << "?Warning: currently running combi_ff version " << combi_ff::current_version
                << " but atom type file " << fragFileName
                << " is version " << version << "\n";
    ...
    for (auto && child : root.children) {
      ...
      AddFragment(tbl_fragments, fragments_to_use, child, atom_type_set, version);
    }
  }

  //Check that every fragment given in fragments_to_use was actually found and added to tbl_fragments
    ...
    for (size_t i = 0; i < tbl_fragments.size(); i++) {
      ...
      if (tbl_fragments.at(i).GetCode() == "") {
        throw input_error("fragment " + fragments_to_use.at(i) +
                          " which is listed in fragments_to_use was not found in any of the specified fragment files\n");
      }
    }
  }
...
void AddFragment(std::vector<TblFragment>& tbl_fragments,
                 ...
                 XmlElement_ptr fragment,
                 const AtomTypeSetMap& atom_type_set, const std::string& version) {
  CheckFragmentXML(fragment);
  std::string code = fragment->attributes["code"];

  if (code.substr(0, TblFragment::fragment_prefix.size()) != TblFragment::fragment_prefix)
    throw input_error("Please use prefix " + TblFragment::fragment_prefix +
                      " in front of fragment codes. Missing prefix for " + code + '\n');

  std::vector<TblAtom> atom_types;
  size_t N(0);
  FragmentMatrixTbl M;
  std::vector<std::shared_ptr<TopologicalPropertyBase>> topological_properties(0);
  ...
  for (auto && topological_propertyList : fragment->children) {
    if (topological_propertyList->tag == "atoms") {
      ...
      if (!N)
        throw input_error("defining bonds, but no atoms found in fragment " + code + "\n");

      AddBonds(topological_propertyList, atom_types, M, code);
      AddTopologicalProperties(topological_propertyList, topological_properties, atom_types, code);
  ...
  if (fragments_to_use.size()) {
    auto&& it = std::find(fragments_to_use.begin(), fragments_to_use.end(), code);

    if (it != fragments_to_use.end())
      tbl_fragments.at(std::distance(fragments_to_use.begin(), it)) = (TblFragment(version, code, N,
                                                                                  atom_types,
                                                                                  topological_properties, M));

  } else
    tbl_fragments.push_back(TblFragment(version, code, N, atom_types, topological_properties, M));
}
  ...
  for (auto && atom : atoms->children) {
    ...
    for (auto && atom : atom_types) {
      if (atom.GetAtomID() == new_atom.GetAtomID())
        throw input_error("listed more than one atom with id " + new_atom.GetAtomID() + " in a fragment\n");
    }
...
void SetLinkageType(const XmlElement_ptr link_type, TblAtom& new_atom, bool& is_core_atom) {
  ...
  if (linkaGetype == "core") {
    ...
    if (new_atom.GetAtomTypes().GetAtomSet().size() != 1)
      throw input_error("core atom " + new_atom.GetAtomID() +
...
void CheckAtomTypeXML(const XmlElement_ptr atomtype, const AtomTypeSetMap& atom_type_set,
  ...
  if (it == atom_type_set.end())
    throw input_error("couldn't find atom type (Set) " + atomtype->value + " in atom type Set\n");

  if (it->second.GetVersion() != version)
    std::cout << "?Warning: current fragment file is version " << version
              << ", but atom type (Set) " << atomtype->value
              << " is version " << it->second.GetVersion() << "\n";
}
...
void CheckAtomXML(const XmlElement_ptr atom) {
  ...
  if (!(id.front() >= 'a' && id.front() <= 'z'))
    throw input_error("Please start atom ids with lower case letter in <atom> tag. Not the case for atom "
                      + id + '\n');
}

void AddBonds(const XmlElement_ptr bonds, std::vector<TblAtom>& atom_types, FragmentMatrixTbl& M,
              const std::string& code) {
  CheckBondsXML(bonds);
  ...
  for (auto && bond : bonds->children) {
    ...
    std::vector<size_t> involved_atomsIdx = CreateInvolvedAtomIndexVector(involved_atoms,
                                                                          atom_types,
                                                                          code);
...
std::vector<size_t> CreateInvolvedAtomIndexVector(const XmlElement_ptr involved_atoms,
  ...
  for (auto && involved_atom : involved_atoms->children) {
    ...
    if (!found)
      throw input_error("atom with id " + atomID + " not found in atoms list in fragment " + code + "\n");
  }
...
void CheckBondValidity(TblAtom& atom1, TblAtom& atom2, const double degree) {
  if (atom1.GetLinkageType() == link && atom2.GetLinkageType() == link)
    throw input_error("not allowed to link 2 link atoms toGether: " + atom1.GetAtomID() + " and " +
                      atom2.GetAtomID() + '\n');
...
void CheckInvolvedAtomsXML(const XmlElement_ptr involved_atoms, const size_t num_involved_atoms) {
...
void CheckBondXML(const XmlElement_ptr bond) {
  ...
  if (bond_degrees.find(degree) == bond_degrees.end())
    throw input_error("unknown bond degree " + degree +
                      " (options are single, double, triple, quadruple, aromatic)\n");
}

void AddTopologicalProperties(const XmlElement_ptr topological_propertyList,
  ...
  for (auto && topological_property : topological_propertyList->children) {
    ...
    topological_properties.push_back(CreateNewTopologicalProperty(topologicalPropertyMap.find(
                                                                    topological_property->tag)->second));
    std::shared_ptr<TopologicalPropertyBase>& curType = topological_properties.back();
    ...
    curType->SetInvolvedAtoms(CreateInvolvedAtomIndexVector(involved_atoms, atom_types, code));
  }
}
...
void CheckTopologicalPropertyXML(const XmlElement_ptr topological_property) {
  ...
  if (topological_property->attributes.size() && topological_property->tag != "bond") {
    topological_property->CheckAttributeSize(1);
    topological_property->CheckAttribute("parameter");
  }

  if (topologicalPropertyMap.find(topological_property->tag) == topologicalPropertyMap.end())
    throw input_error(topological_property->tag + " is not a known topological property\n");

...
IOFileProperties::IOFileProperties() : fie_families(StringVector(0)),
  input_file_names(FileNameVector(num_input_files)),
  output_dir(""), output_dir_molecule_decompositions(""), output_dir_molecules_with_macros(""),
  output_dir_mtb(""),
...
InputOutput::InputOutput(const std::vector<std::string>& arguments) :
  fragments_to_use(std::vector<std::string>(0)),
  arguments(arguments) {
  i = 0;
  ReadInput();
}
...
void InputOutput::ReadInput() {
  //read arguments and open corresponding input and output files. use input files to define required properties
  ReadArguments();
}
  ...
  for (i = 0; i  < arguments.size(); i++) {
    ...
    //command line option -fragments
      ...
      while (i + 1 < arguments.size()) {
        ...
        } else io_file_properties.fie_families.push_back(arg);
      }
    }
    ...
    //command line optoin -outputDir
      ...
      if (io_file_properties.output_dir_molecule_decompositions.size()) {
        std::cout << "?Warning: using output directory for molecule decompositions '" <<
                  io_file_properties.output_dir_molecule_decompositions
                  << "'. ignoring general output directory '" << io_file_properties.output_dir << "'\n";
      }

      if (io_file_properties.output_dir_molecules_with_macros.size()) {
        std::cout << "?Warning: using output directory for molecules with macros '" <<
                  io_file_properties.output_dir_molecules_with_macros
                  << "'. ignoring general output directory '" << io_file_properties.output_dir << "'\n";
      }

      if (io_file_properties.output_dir_mtb.size()) {
        std::cout << "?Warning: using output directory for mtb '" << io_file_properties.output_dir_mtb
                  << "'. ignoring general output directory '" << io_file_properties.output_dir << "'\n";
      }
    ...
    //command line option -fragments_to_use
      ...
      throw input_error("Unrecognized input option " + arg + ". See above for possible options\n");
    }
  }
}
...
void InputOutput::ReadFileNames(size_t& i, const InputFileType input_type) {
  const std::string& name = io_file_properties.input_file_name_building_blocks[input_type];
  std::string line;
...
void InputOutput::GetInputFile() {
  ...
  if (!arguments_file.is_open())
    throw std::runtime_error(arg + " not open\n");
...
void InputOutput::PrintInputOptions() {
  std::cout << "input options are:\n\n"
            ...
            << "-fragments: used to specify the input file(s) in which the Topology Builder Fragments are defined. Several files can be listed in a row\n"
            << "   - defaults to ./fragments/fragments_alkanes, if not specified\n\n"
            << "-fragments_to_use: used to specify the Fragments to use by listing the fragment codes\n"
            ...
            << "-unique_torsionals: should only one torsional dihedral per central bond be added to the topology?\n\n"
            << "-output_directory: used to specify the path where general output files should be stored\n\n"
            << "-output_directory_decompositions: used to specify the path where the molecule decomposition files should be stored\n\n"
            << "-output_directory_molecule_macros: used ot specify the path where the molecules with macros files should be stored\n\n"
            << "-output_directory_mtb: used to specify the path where the mtb files should be stored\n\n";
}

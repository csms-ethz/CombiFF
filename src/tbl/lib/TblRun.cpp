...
TblRun::TblRun(const std::vector<std::string>& arguments) : arguments(arguments) {}
...
void TblRun::run() {
  ...
  CreateAtomTypeSets(io_file_properties.input_file_names[atomtypes_file], atom_type_set);
  ...
  CreateTblFragments(io_file_properties.input_file_names[fragment_file],
                     tbl_fragments,
                     io.GetFragmentsToUse(),
                     atom_type_set);

  for (auto && filename_familyIsomerEnumeration : io_file_properties.fie_families) {
    std::cout << "\n\n"
              << "**********************************************************\n"
              << "family isomer enumeration file is " << filename_familyIsomerEnumeration << '\n'
              << "**********************************************************\n\n";
    FamilyDecomposer decomposer(filename_familyIsomerEnumeration, io_file_properties, tbl_fragments);
    std::string filename_molecules_with_macros("");
    CreateMoleculesWithMacros(decomposer.GetFilename(), filename_molecules_with_macros,
                              decomposer.GetFamilyCode(), tbl_fragments, io_file_properties);
    CreateMTB(filename_molecules_with_macros, decomposer.GetFamilyCode(), io_file_properties);
  }
}

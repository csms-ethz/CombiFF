...
class MoleculeDecomposer : private FamilyDecomposer {
 ...
 public:
  ...
  bool UllmannMatch(ComparisonMatrix& M,
                    std::vector<bool>& matched_columns,
  ...
  bool Refine(combi_ff::ComparisonMatrix& M,
              int k,
              const size_t n,
              const size_t m,
              const FragmentMatrixTbl& fragment_matrix,
 ...
 private:
  ...
  std::vector<std::vector<const TblAtom*>> link_atoms {std::vector<std::vector<const TblAtom*>>(0)};

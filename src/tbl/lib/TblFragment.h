...
static BondDegreeMap bond_degrees{{"single", 1}, {"double", 2}, {"triple", 3}, {"quadruple", 4}, {"aromatic", 1.5}};
...
class TblFragment {
 public:
  ...
  const std::vector<std::shared_ptr<TopologicalPropertyBase>>& GetTopologicalProperties() const;
 ...
 private:
  ...
  std::vector<const TblAtom*> core_atoms {std::vector<const TblAtom*>(0)}; //if an atom in atom_types is a core atom, core_atoms points to it, if it's a link atom it points to the bonded core atom
  std::vector<std::shared_ptr<TopologicalPropertyBase>> topological_properties {std::vector<std::shared_ptr<TopologicalPropertyBase>> (0)};
  size_t n_single {0};
  size_t n_aromatic {0};
  size_t n_double {0};
  size_t n_triple {0};
...
void AddAtoms(const XmlElement_ptr atoms,
              const AtomTypeSetMap& atom_type_set,
              std::vector<TblAtom>& atom_types,
              const std::string& version);
void AddBonds(const XmlElement_ptr bonds,
              std::vector<TblAtom>& atom_types,
              FragmentMatrixTbl& M ,
              const std::string& code);
void AddTopologicalProperties(const XmlElement_ptr topological_property,
                              std::vector<std::shared_ptr<TopologicalPropertyBase>>& topological_properties,
                              std::vector<TblAtom>& atom_types,
                              const std::string& code);
void SetLinkageType(const XmlElement_ptr link_type,
                    TblAtom& new_atom,
                    bool& is_core_atom);

...
constructor
*/
AtomInMolecule::AtomInMolecule(size_t idx,
...
void CreateMoleculesWithMacros(const std::string& filename_molecule_decomposition,
  ...
  filename_molecules_with_macros = ((io_file_properties.output_dir_molecules_with_macros.size() ?
                                     io_file_properties.output_dir_molecules_with_macros : io_file_properties.output_dir)
                                    + "molecules_with_macros_" + family_code + ".xml");
  size_t counter(0);
  XmlParserOut molecules_macros(filename_molecules_with_macros, "molecules_with_macros");
  molecules_macros.GetRoot().attributes["tbl_version"] = combi_ff::current_version;
  ...
  //for(auto && molecule_decomposition : molecule_decompositions_in.GetRoot().children) {
  while (molecule_decompositions_in.ReadUntilElement("molecule_decomposition")) {
    auto&& molecule_decomposition = molecule_decompositions_in.GetRoot().GetLastChild();
    ...
    CreateMoleculeWithMacros(molecule_decomposition, molecule_macros, tbl_fragments,
                             io_file_properties.united_atom, io_file_properties.third_neighbor_exclusions,
...
void CreateMoleculeWithMacros(XmlElement_ptr molecule_decomposition, XmlElement& molecule_macros,
                              const std::vector<TblFragment>& tbl_fragments,
                              const bool united_atom, const bool third_neighbor_exclusions, const bool unique_torsionals) {
  ...
  std::unordered_map<std::string, std::pair<std::string, std::string>> core_atom_types;
  std::vector<std::vector<topo>> topological_properties(num_topological_property_types);
  ...
  for (auto && fragment : fragments->children) {
    ...
    if (used_fragments.find(fragment_id) != used_fragments.end())
      throw std::runtime_error("fragment ID "
                               + fragment_id
      ...
      if (frag.GetCode() == fragCode) {
        ...
        if (frag.GetVersion() != combi_ff::current_version)
          std::cout << "?Warning: currently running combi_ff version " << combi_ff::current_version
                    << " but fragment " << fragCode
                    << " has version " << frag.GetVersion() << "\n";

        for (auto && TBatom : frag.GetTblAtoms()) {
          if (TBatom.GetLinkageType() == core) {
            atms.push_back({fragment_id + "_" + TBatom.GetAtomID(), &(TBatom.GetAtomTypes())});
          }

          core_atom_types[fragment_id + "_" + TBatom.GetAtomID()] = std::pair<std::string, std::string>
                                                                    (fragment_id + "_" + TBatom.GetAtomID(),
                                                                     frag.GetCoreAtom(TBatom.GetAtomID())->GetAtomTypes().GetName());
        }
        ...
        for (auto && prop : frag.GetTopologicalProperties()) {
          ...
          topological_properties[topologicalPropertyMap.find(prop->GetPropertyName())->second].push_back(
            newtopo);
        }
    ...
    if (!found)
      throw std::runtime_error("fragment " + fragment_id + " not found in tbl_fragments\n");
  }
  ...
  for (auto && linkage : linkages->children) {
    ...
    for (auto && involved_fragment : linkage->children) {
      ...
      frags.push_back(used_fragments[involved_fragment->GetFirstChild()->value]);
    ...
    core_atom_types[*fragment_ids[0] + "_" + involved_atoms[0]].first = *fragment_ids[1]
                                                                        + "_"
    ...
    core_atom_types[*fragment_ids[1] + "_" + involved_atoms[1]].first = *fragment_ids[0]
                                                                        + "_"
    ...
    connected_fragments[*fragment_ids[0] + "_" + frags[0]->GetCoreAtom(
                          involved_atoms[0])->GetAtomID()].push_back(frags[1]->GetCode()
                                                                     + "."
                                                                     + involved_atoms[1]);
    connected_fragments[*fragment_ids[1] + "_" + frags[1]->GetCoreAtom(
                          involved_atoms[1])->GetAtomID()].push_back(frags[0]->GetCode()
                                                                     + "."
                                                                     + involved_atoms[0]);
  }
  ...
  for (auto && cc : connected_fragments) {
    //std::sort(cc.second.begin(), cc.second.end()); // don't sort, as fragments arent necessarily symmetric
  ...
  size_t idx = 1;
  atoms->GetLastChild()->attributes["atom_id"] = (at.second->GetAtomSet().begin()->GetElementSymbol())
                                                 + std::to_string(idx);
  ...
  atoms_in_molecule.emplace(at.first, AtomInMolecule(0, at.second->GetName(),
                                                     (at.second->GetAtomSet().begin()->GetElementSymbol()) + std::to_string(idx),
                                                     connected_fragments[at.first]));
  indexed_atoms.push_back(AtomInMolecule(0, at.second->GetName(),
                                         (at.second->GetAtomSet().begin()->GetElementSymbol()) + std::to_string(idx),
                                         connected_fragments[at.first]));
  ...
  for (size_t i = 1; i < atms.size(); i++) {
    auto&& at = atms[i];
    atoms->AddElement("atom");
    //if (atms[i - 1].second->GetAtomSet().begin()->GetElementSymbol() != at.second->GetAtomSet().begin()->GetElementSymbol())
    //  idx = 1;
    atoms->GetLastChild()->attributes["atom_id"] = (at.second->GetAtomSet().begin()->GetElementSymbol())
                                                   + std::to_string(idx);
    atoms->GetLastChild()->attributes["atom_type"] = at.second->GetName();
    ...
    atoms_in_molecule.emplace(at.first, AtomInMolecule(i, at.second->GetName(),
                                                       (at.second->GetAtomSet().begin()->GetElementSymbol()) + std::to_string(idx),
                                                       connected_fragments[at.first]));
    indexed_atoms.push_back(AtomInMolecule(i, at.second->GetName(),
                                           (at.second->GetAtomSet().begin()->GetElementSymbol()) + std::to_string(idx),
                                           connected_fragments[at.first]));
    indexed_atoms.back().atomtype = core_atom_types[at.first].second;
    idx++;
  ...
  std::vector<std::list<std::shared_ptr<TopologicalPropertyBase>>> topological_properties_in_molecule(
    num_topological_property_types);

  for (auto && prop_type : topological_properties) {
    if (prop_type.size()) {
      PropertyType type = topologicalPropertyMap.find(
                            prop_type.front().property->GetPropertyName())->second;

      for (auto && prop : prop_type) {
        topological_properties_in_molecule[type].push_back(CreateNewTopologicalProperty(type));
        auto&& cur_property = topological_properties_in_molecule[type].back();

        if (type == bond)
          cur_property->AddAttribute("degree", prop.property->GetAttribute("degree"));

        for (auto && idx : prop.property->GetInvolvedAtoms()) {
          auto&& atom = prop.frag->GetTblAtom(idx);
          cur_property->AddInvolvedAtom(atoms_in_molecule[core_atom_types[prop.fragment_id + "_" +
                                                                          atom.GetAtomID()].first].idx);
        }
        ...
        for (auto && idx : cur_property->GetInvolvedAtoms()) {
          for (auto && idx_ : prop.property->GetInvolvedAtoms()) {
            ...
            if (atoms_in_molecule[core_atom_types[prop.fragment_id + "_" + atom.GetAtomID()].first].idx ==
                idx) {
              involved_atoms_codes.push_back(core_atom_types[prop.fragment_id + "_" + atom.GetAtomID()].first);
              break;
            }
          }
        }

        std::string parameter_name = (prop.property->GetAttribute("parameter").size() ?
                                      prop.property->GetAttribute("parameter") + "_" : "");
        parameter_name += GetParameterNameFromFragment(*prop.frag, prop.fragment_id, *prop.property,
                                                       core_atom_types, involved_atoms_codes, connected_fragments);
      ...
      if (topological_properties_in_molecule[type].size() > 1) {
        ...
        for (; it_next != topological_properties_in_molecule[type].end(); it++) {
  ...
  for (auto && bd : topological_properties_in_molecule[bond]) {
    ...
    if (bond_degree == "single")deg = 1;

    else if (bond_degree == "double") deg = 2;

    else if (bond_degree == "triple")  deg = 3;

    else if (bond_degree == "quadruple")  deg = 4;

    else if (bond_degree == "aromatic")  deg = 1.5;

    else throw std::runtime_error("unknown bond degree " + bond_degree + "\n");

    A.SetElement(bd->GetInvolvedAtoms()[0], bd->GetInvolvedAtoms()[1], deg);
    indexed_atoms[bd->GetInvolvedAtoms()[0]].neighbors.push_back(bd->GetInvolvedAtoms()[1]);
    indexed_atoms[bd->GetInvolvedAtoms()[1]].neighbors.push_back(bd->GetInvolvedAtoms()[0]);
  ...
  /*for (auto && bd : topological_properties_in_molecule[bond]) {
    ...
    auto&& cur_first_neighbor = topological_properties_in_molecule[first_neighbor].back();
    ...
    std::string parameter_name = GetParameterNameAutomatically(A, cur_first_neighbor, indexed_atoms, core_atom_types);
  ...
  for (auto && bd : topological_properties_in_molecule[bond]) {
    if (indexed_atoms[bd->GetInvolvedAtoms()[0]].neighbors.size() > 1) {
      //angle(s) found
      for (auto && nbr_idx : indexed_atoms[bd->GetInvolvedAtoms()[0]].neighbors) {
        if (nbr_idx != indexed_atoms[bd->GetInvolvedAtoms()[1]].idx) {
          topological_properties_in_molecule[angle].push_back(CreateNewTopologicalProperty(angle));
          auto&& cur_angle = topological_properties_in_molecule[angle].back();
          cur_angle->AddInvolvedAtom(indexed_atoms[bd->GetInvolvedAtoms()[1]].idx);
          cur_angle->AddInvolvedAtom(indexed_atoms[bd->GetInvolvedAtoms()[0]].idx);
          cur_angle->AddInvolvedAtom(nbr_idx);
          cur_angle->SortInvolvedAtoms();
          std::string parameter_name = GetParameterNameAutomatically(A, cur_angle, indexed_atoms,
                                                                     core_atom_types);
          ...
          auto&& cur_second_neighbor = topological_properties_in_molecule[second_neighbor].back();
          ...
          parameter_name = GetParameterNameAutomatically(A, cur_second_neighbor, indexed_atoms, core_atom_types);
    ...
    if (indexed_atoms[bd->GetInvolvedAtoms()[1]].neighbors.size() > 1) {
      //angle(s) found
      for (auto && nbr_idx : indexed_atoms[bd->GetInvolvedAtoms()[1]].neighbors) {
    ...
    if (indexed_atoms[bd->GetInvolvedAtoms()[0]].neighbors.size() > 1 &&
      ...
      //auto&& cur_torsional_dihedral = topological_properties_in_molecule[torsional_dihedral].back();
      // auto&& cur_third_neighbor = topological_properties_in_molecule[third_neighbor].back();
      for (const auto nbr1 : indexed_atoms[bd->GetInvolvedAtoms()[0]].neighbors) {
        for (const auto nbr2 : indexed_atoms[bd->GetInvolvedAtoms()[1]].neighbors) {
          if (nbr1 != indexed_atoms[bd->GetInvolvedAtoms()[1]].idx &&
              nbr2 != indexed_atoms[bd->GetInvolvedAtoms()[0]].idx) {
            topological_properties_in_molecule[torsional_dihedral].push_back(CreateNewTopologicalProperty(
                                                                               torsional_dihedral));
            auto&& cur_torsional_dihedral = topological_properties_in_molecule[torsional_dihedral].back();
            ...
            std::string parameter_name = GetParameterNameAutomatically(A, cur_torsional_dihedral, indexed_atoms,
                                                                       core_atom_types);
      ...
      /*
      if (indexed_atoms[bd->GetInvolvedAtoms()[0]].neighbors[0] == indexed_atoms[bd->GetInvolvedAtoms()[1]].idx){
      ...
      if (indexed_atoms[bd->GetInvolvedAtoms()[1]].neighbors[0] == indexed_atoms[bd->GetInvolvedAtoms()[0]].idx){
      ...
      //parameter_name = GetParameterNameAutomatically(A, cur_third_neighbor, indexed_atoms, core_atom_types);
  ...
  */
  if (united_atom) {
    ...
    for (; it != topological_properties_in_molecule[bond].end(); it++) {
      ...
      if (!topological_properties_in_molecule[bond].size())
        break;
    }
  }

  /*
  remove duplicate topological properties (e.g. angle a-b-c = c-b-a, after sorting)
  */
  for (int property = angle; property < num_topological_property_types; property++) {
    if (topological_properties_in_molecule[property].size() > 1) {
      ...
      for (; it_next != topological_properties_in_molecule[property].end(); it++) {
        if ((*it)->GetInvolvedAtoms() == (*it_next)->GetInvolvedAtoms()) {
    ...
    if (united_atom) {
      ...
      for (; it != topological_properties_in_molecule[property].end(); it++) {
        ...
        if (!topological_properties_in_molecule[property].size())
          break;
      }
    }
  }
  ...
  if (unique_torsionals) {
    if (topological_properties_in_molecule[torsional_dihedral].size() > 1) {
      for (auto it = topological_properties_in_molecule[torsional_dihedral].begin();
           it != topological_properties_in_molecule[torsional_dihedral].end(); it++) {
        auto it_next = it;
        it_next++;

        for (; it_next != topological_properties_in_molecule[torsional_dihedral].end(); it_next++) {
          if ((*it)->GetInvolvedAtoms()[1] == (*it_next)->GetInvolvedAtoms()[1]
              && (*it)->GetInvolvedAtoms()[2] == (*it_next)->GetInvolvedAtoms()[2]) {
            auto it_prev = it_next;
            it_prev--;
            topological_properties_in_molecule[torsional_dihedral].erase(it_next);
            it_next = it_prev;
          }
        }
      }
  ...
  if (united_atom) {
    for (auto && at_it = atoms->children.begin(); at_it != atoms->children.end(); at_it++) { // atoms
      if ((*at_it)->attributes["atom_type"].find("united") != std::string::npos) {
        auto erase_it = at_it;
        at_it--;
  ...
  if (!third_neighbor_exclusions) {
    for (auto && at : atoms->children) {
      ...
      excluded_atoms.erase(std::unique(excluded_atoms.begin(), excluded_atoms.end()),
                           excluded_atoms.end());
    ...
    for (auto && at : atoms->children) {
      ...
      for (auto && tors : topological_properties_in_molecule[torsional_dihedral]) {
        if (indexed_atoms[tors->GetInvolvedAtoms().front()].atom_id == atom_id
...
const std::string GetParameterNameFromFragment(const TblFragment& frag,
  ...
  if (prop.GetInvolvedAtoms().size()) {
    parameter_name += "_"
                      + core_atom_types.find(fragment_id + "_" + frag.GetTblAtom(
                                               prop.GetInvolvedAtoms()[0]).GetAtomID())->second.second;
    PropertyType type = topologicalPropertyMap.find(prop.GetPropertyName())->second;
    ...
    if (type != improper_dihedral) {
      for (size_t i = 0; i < prop.GetInvolvedAtoms().size() - 1; i++) {
        ...
        parameter_name += core_atom_types.find(fragment_id + "_" + frag.GetTblAtom(
                                                 idx2).GetAtomID())->second.second;
      }
...
// create the parameter name for automatically determined properties (currently: angles and torsionals)
const std::string GetParameterNameAutomatically(const topology_builder::AdjacencyMatrix& A,
  ...
  parameter_name += indexed_atoms[cur_property->GetInvolvedAtoms()[0]].parameter;

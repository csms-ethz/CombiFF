...
Pseudoatom::Pseudoatom(const FragmentMatrix& M, const std::string& code,
...
GetTER FUNCTIONS
***************/
const FragmentMatrix& Pseudoatom::GetMatrix() const { return M; }

const std::string& Pseudoatom::GetCode() const { return code; }

const std::string& Pseudoatom::GetVersion() const { return version; }

size_t Pseudoatom::GetDegree() const { return degree; }
...
void CreatePseudoatoms(PseudoatomMap& pseudoatoms,
  ...
  for (auto&& pseudoatom_file_name : pseudoatom_file_names) {
    ...
    const std::string& version =
        pseudoatoms_root.attributes.find("version")->second;

    if (version != combi_ff::current_version)
      std::cout << "?Warning: currently running combi_ff version "
                << combi_ff::current_version << " but pseudoatom file "
                << pseudoatom_file_name << " is version " << version << "\n";
...
void GetNextPseudoatom(PseudoatomMap& pseudoatoms,
  ...
  const std::string& code =
      "'" + pseudoatom->attributes.find("code")->second + "'";

  if (pseudoatoms.find(code) != pseudoatoms.end())
    throw input_error("pseudoatom with code " + code +
  ...
  for (auto&& atom_xml : atoms_xml->children) {
    ...
    if (atom_xml->value == "H")
      throw combi_ff::input_error(
          "In " + code +
          ": Pseudoatoms cannot contain explicit hydrogens. Please use "
          "implicit hydrogen notation if you wish to include hydrogen atoms in "
          "the pseudoatom");
  ...
  while (adjacency_stack_stream >> value) v.push_back(value);
  ...
  for (size_t i = 0; i < M.GetN() - 1; i++) {
    for (size_t j = i + 1; j < M.GetN(); j++) {
      if (idx >= v.size())
        throw combi_ff::input_error(
            "stack too small for number of atoms given for pseudoatom " + code);
  ...
  if (idx != v.size())
    throw combi_ff::input_error(
        "stack too big for number of atoms given for pseudoaotm " + code);

  if (M.GetAtomVector().front().GetDegree() <= M.AccumulateRow(0))
    throw combi_ff::input_error(
        "first atom of the pseudoatom is already fully linked for pseudoatom" +
        code);
  ...
  for (size_t row = 1; row < M.GetN(); row++) {
    if (M.GetAtomVector()[row].GetDegree() != M.AccumulateRow(row))
      throw combi_ff::input_error("atom " + std::to_string(row) +
                                  " is not fully linked (either too many or "
                                  "not enough bonds) for pseudoatom " +
                                  code);
  }

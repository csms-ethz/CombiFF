...
AtomTypeAlias::AtomTypeAlias(const std::string& alias_name, const std::string& version) :
	alias_name(alias_name), contained_atom_types(std::vector<combi_ff::ElementSymbol>(0)), version(version) {}

const std::string& AtomTypeAlias::GetVersion() const {
	return version;
}
const std::string& AtomTypeAlias::GetAliasName() const {
	return alias_name;
}
...
const std::vector<combi_ff::ElementSymbol>& AtomTypeAlias::GetContainedAtomTypesConst() const {
	return contained_atom_types;
}
void AtomTypeAlias::SetVersion(const std::string& v) {
	version = v;
}
void AtomTypeAlias::SetAliasName(const std::string& a) {
	alias_name = a;
}
...
void GetNextAlias(AliasMap& alias_map, const XmlElement_ptr alias, const std::string& version) {
	...
	alias_map[map_name].GetContainedAtomTypes().reserve(alias->GetNumberOfChildren());
	...
	for(auto && member : alias->children) {
		...
		if(alias_map.find(member->value) != alias_map.end())
			alias_map[map_name].GetContainedAtomTypes().insert(alias_map[map_name].GetContainedAtomTypes().end(),
	 ...
	 while(j < map.size()) {
	     ...
	     do {
	         ...
	         //case that the next element is a united atom type
	             ...
	             if(ele != "")
	                 throw combi_ff::input_error("didn't expect an alphabetical character before \'{\' in " + map);
	     ...
	     std::cout << "  -" << std::setw(5) << std::left << map_name << " [" << elements << "]\n";
	 }*/
}

/************************************************************************************
SORT THE ELEMENTS IN THE VECTOR OF A ALIASES FIRST BY DEGREE, AND THEN ALPHABETICALLY
************************************************************************************/
...
bool CompareElems(std::string e1, std::string e2) {
	...
	if(e1[0] == '{')
		e1 = e1.substr(1, e1.size() - 1);

	if(e2[0] == '{')
		e2 = e2.substr(1, e2.size() - 1);

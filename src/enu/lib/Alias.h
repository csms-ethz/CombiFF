...
class AtomTypeAlias {
 public:
  ...
  const std::vector<combi_ff::ElementSymbol>& GetContainedAtomTypesConst()
      const;
 ...
 private:
  ...
  std::vector<combi_ff::ElementSymbol> contained_atom_types{
      std::vector<combi_ff::ElementSymbol>(0)};
  std::string version{""};
};

...
CONSTRUCTORS
***********/
Family::Family(const std::string& code,
  ...
  code(code), version(version), used_atom_vectors(used_atom_vectors),
  lambda_ranges_vec(lambda_ranges_vec),
  max_degree(max_degree), ranged_properties(ranged_properties),
  substructures(substructures), has_pseudoatom(has_pseudoatom) {}

/*************
GetTER METHODS
*************/
const std::string& Family::GetCode() const {
  return code;
}

const std::string& Family::GetVersion() const {
  return version;
}

const std::vector<AtomVector<combi_ff::Atom>>& Family::GetUsedAtomVectors()const {
...
bool Family::GetHasPseudoatom() const {
  return has_pseudoatom;
}
...
/****************************************************************************************************
CREATE THE VECTOR OF families FROM THE familyDefinitionFile. TO THIS END, FIRST CREATE TEMPLATE FAMILIES
FROM THE DEFINITONS IN THE familyDefinitionFile, AND THEN GO THROUGH used_families TO ADD THE REQUIRED
FAMILIES
****************************************************************************************************/
...
void CreateFamilies(FamilyVector& families,
                    ...
                    const AliasMap& alias_map, const PseudoatomMap& pseudoatoms) {
  if (!family_definition_file_names.size()) {
    if (used_families.size()) {
      std::cout << used_families << '\n';
      throw std::runtime_error("specified the above families to enumerate, but no family Set file is open\n");
...
//only create families if they are used
  if (used_families.size()) {
    ...
    for (auto && family_definition_file_name : family_definition_file_names) {
      ...
      const std::string& version = families_root.attributes.find("version")->second;

      if (version != combi_ff::current_version)
        std::cout << "?Warning: currently running combi_ff version " <<
                  combi_ff::current_version
                  << " but family file " << family_definition_file_name
                  << " is version " << version << "\n";

      bool all_fam(false);

      if (std::find(used_families.begin(), used_families.end(), "all") != used_families.end()) {
      ...
      //line >> blockName;
      //control vector for code and acronym to check that each code and each acronym occurs only once
      StringVector codes(0), acros(0);

      //while(!familyDefinitionFile.eof())
      for (auto && family : families_root.children)
        GetNextFamily(version, families, family, used_families, alias_map, pseudoatoms,
                      abstr_substructures, all_fam, codes, acros);
    }
  }
    ...
    if (f != "all") {
      ...
      if (!found)
        throw combi_ff::input_error("specified family '" + f +
                                    "' in input, but couldn't find this family in the family library file(s)");
    }
  }
...
/******************************************************************************************************************
CREATE A NEW FAMILY BY READING A FAMILY BLOCK IN familyDefinitionFile BETWEEN THE NEWFAMILY TO THE ENDFAMILY KEYWORDS,
BUT IF A FAMILY ACRO/CODE IS NOT ENCOUNTERED IN THE used_families ACRO/CODE, THE FAMILY CAN BE SKIPPED
******************************************************************************************************************/
void GetNextFamily(const std::string& version, FamilyVector& families,
                   ...
                   const AbstractSubstructureMap& abstr_substructures, const bool all_fam,
                   StringVector& codes, StringVector& acros) {
  ...
  std::vector<AtomVector<combi_ff::Atom>> used_atom_vectors(1, AtomVector<combi_ff::Atom>(0));
  ...
  if (std::find(codes.begin(), codes.end(), code) != codes.end())
    throw combi_ff::input_error("family with code " + code +
                                " occurs more than once in the family Set file");

  if (!all_fam &&
      std::find(used_families.begin(), used_families.end(),
                code) == used_families.end()) {
  ...
  for (; family_property_it != family->children.end(); ++family_property_it) {
    ...
    if (tag_name == "max_bond_degree") {
      ...
      for (auto && c : family_property->value) {
        if (!std::isdigit(c))
          throw input_error("maximum_bond_degree has to be an integer value, but " +
                            std::string(1, c) + " in " + family_property->value + "is not a digit\n");
      }
    ...
    //read SUBSTRUC block
      ...
      GetSubstructures(family_property, abstr_substructures, substructures, alias_map,
                       version);

    //unknown input
    else
      throw combi_ff::input_error("in family file, unrecognized option " + tag_name +
  ...
  if (all_fam ||
    ...
    families.push_back(Family(code, version, used_atom_vectors, lambda_ranges_vec,
                              max_degree,
                              ranged_properties, substructures, has_pseudoatom));
  }
}


/******************************************************************************************
READ THE FORMULA BLOCK OF THE familyDefinitionFile AND ADD THE CORRESPONDING ATOMS AND LAMBDAS
 -> note that unlike in the simple processIO.cpp input, the atom types can also be alias_map
    here. Additionally, different alias atom types can be defined as XOR to each other
    -> note that if you want to use AND, just use MAPn, to use n of the same atom types
       and if you want to use OR, just use MAPn Mapm ..., to use n of the same and m of the
       same or a different type
 -> each AtomVector has a vector of LambdaVectors that it corresponds to
 -> every time, a alias is encountered, the number of used_atom_vectors and lambda_ranges_vec
    is increased by a factor corresponding to the size of the alias, since they're all
    stored explicitly
 -> xor_idx: has for every atom type. if an atom type doesn't have an XOR indicator ^, the
    entry is -1. if two atom types have the same entry, they *have* to be different
******************************************************************************************/
void GetFormula(const std::string& formula_, const AliasMap& alias_map,
                const PseudoatomMap& pseudoatoms,
                std::vector<AtomVector<combi_ff::Atom>>& used_atom_vectors,
                std::vector<std::list<LambdaVector>>& lambda_ranges_vec, bool& has_pseudoatom,
                const std::string& version) {
  if (lambda_ranges_vec.size())
    throw combi_ff::input_error("please only define one FORMULA entry per family");
  ...
  while (j < formula.size()) {
    ...
    //case that the current atom type is a united atom
      ...
      has_pseudoatom = true;
      //add the atom type and lambda value
      AddPseudoatom(formula, j, used_atom_vectors, lambda_ranges, pseudoatoms, version);
      ...
      throw combi_ff::input_error("format error for \"FORMULA\" in Family: unexpected character "
                                  + std::string(1, formula[j]) + " in " + formula);
  }

  CleanUpAtomVectorsFam(has_xor, xor_idx, lambda_ranges, used_atom_vectors, lambda_ranges_vec);
}


/**************************************************************************************************************
READ THE RESTRICTION AFTER THE NUNSAT, NBONDS, NSB, NDB, NTB, OR NRINGS KEYWORDS AND CALL readRange TO ADD THEM
**************************************************************************************************************/
...
void GetSubstructures(const XmlElement_ptr substructures_xml,
  ...
  for (auto && substructure : substructures_xml->children) {
    ...
    if (prototype_substr->second.GetVersion() != version)
      std::cout << "?Warning: using substructure " << code
                << " with version " << prototype_substr->second.GetVersion()
                << " but current family is version " << version << "\n";

    const std::string& range_value = substructure->attributes.find("amount")->second;
    Range r({0, -1});
    ReadRange(range_value, r);
    ...
    AddAtomTypes(atom_types_in_substructure, j, xor_idx, and_idx, atom_vectors, alias_map,
                 version);

    for (auto && a : atom_vectors) {
      if (a.size() != prototype_substr->second.GetMatrix().GetN())
        throw combi_ff::input_error("atom vector not given correctly for " + code);
    }
    ...
    if (substructure->attributes.find("repetition") !=
      ...
      if (repetitionType == "and")
        ...
        throw input_error("unknown repetition type " + repetitionType +
                          " for substructure " + code + " (known types are and, xor, or)\n");
    }

    //remove the atom vectors that don't fulfill the AND/XOR restrictions from atom_vectors
    CleanUpAtomVectorsFrag(atom_vectors, xor_idx, and_idx);
    //Add the Fragment whose input parameters were just read to the fragments vector
...
void AddAtom(const std::string& formula, size_t& j, const AliasMap& alias_map,
             std::vector<AtomVector<combi_ff::Atom>>& used_atom_vectors,
             std::list<LambdaVector>& lambda_ranges, const std::string& version) {
  //read the atom name or alias code into nam
  std::string ele("");

  while (j < formula.size() && isalpha(formula[j]))
    ele += formula[j++];
  ...
  //auto it = std::find_if( alias_map.begin(), alias_map.end(), [](const alias& element){ return alias.first == ele;} );
  ...
  if (alias != alias_map.end()) {
    ...
    if (alias->second.GetVersion() != version)
      std::cout << "?Warning: using atom type alias " << ele
                << " with version " << alias->second.GetVersion()
                << " but current family is version " << version << "\n";

    std::vector<AtomVector<combi_ff::Atom>> atom_vectors_tmp(0);

    //each atom vector is added as many times as there are elements in the alias, one copy for each of the element in the alias,
    //where the last element is an alias
    for (size_t ii = 0; ii < used_atom_vectors.size(); ii++) {
      for (auto && alias_atom : alias->second.GetContainedAtomTypesConst()) {
        //add a copy of the previously used used_atom_vectors[ii], and append it with alias_atom
        ...
        if (alias_atom[0] == '{') {
          ...
          while (jj < alias_atom.size() && alias_atom[jj] != '}' && isalpha(alias_atom[jj]))
            ele += alias_atom[jj++];

          //add an Atom of type ele (minus the last 'H' char) to the currently last vector in tempAtomsVecs
          atom_vectors_tmp.back().push_back(ele.substr(0, ele.size() - 1));
          //Get number of H atoms in united atom. Default is 1
          size_t n(1);

          if (isdigit(alias_atom[jj]))
            n = GetNumber(jj, alias_atom);

          //Set the fixed number of H atoms in the Atom that was just added
          atom_vectors_tmp.back().back().SetNumFixedHydrogens(n);
        }

        //if not a unitd atom, directly add the atom to the currently last vector in tempAtomsVecs
        else
  ...
  // if the current element is not an alias, simply add it to all of the vectors in used_atom_vectors
  if (!isAlias) {
...
void AddUnitedAtom(const std::string& formula, size_t& j,
  ...
  if (nam.back() != 'H')
    throw combi_ff::input_error("united atom notation only works for hydrogen atom, e.g. {CH3}");

  //shorten nam, s.t. the 'H' is not part of it anymore, but only the atom type name of the first atom
...
void AddPseudoatom(const std::string& formula, size_t& j,
                   std::vector<AtomVector<combi_ff::Atom>>& used_atom_vectors,
                   std::list<LambdaVector>& lambda_ranges,
                   const PseudoatomMap& pseudoatoms, const std::string& version) {
  std::string nam("\'");
  ...
  if (psa->second.GetVersion() != version)
    std::cout << "?Warning: using pseudoatom " << nam
              << " with version " << psa->second.GetVersion()
              << " but current family is version " << version << "\n";
...
/************************************************************************************************************
REMOVE ANY DUPLICATES (both in AtomVector and LambdaVectors) AND VIOLATIONS OF XOR CONDITIONS FROM THE used_atom_vectors
************************************************************************************************************/

void CleanUpAtomVectorsFam(bool has_xor, std::vector<int>& xor_idx,
                           std::list<LambdaVector>& lambda_ranges,
                           std::vector<AtomVector<combi_ff::Atom>>& used_atom_vectors,
                           std::vector<std::list<LambdaVector>>& lambda_ranges_vec) {
  //create used_atom_vectors.size() copies of xor_idx in a vector xor_idx_vec, s.t. each vector in used_atom_vectors has its own
  //xor_idx vector. This assures that when the vectors are sorted in the next step, the entries of the xor_idx  vector
  //still correspond to the correct atom types for all of the vectors
  std::vector<std::vector<int>>xor_idx_vec(used_atom_vectors.size(), xor_idx);
  //same goes for lambda_ranges, except here they have to be added to the currently existing lambda_ranges_vec;
  std::vector<std::list<LambdaVector>> lambda_ranges_tmp(std::vector<std::list<LambdaVector>>
                                                         (used_atom_vectors.size(), lambda_ranges));
  lambda_ranges_vec.insert(lambda_ranges_vec.end(), lambda_ranges_tmp.begin(),
                           lambda_ranges_tmp.end());

  //sort the vectors in used_atom_vectors, toGether with their corresponding vector of LambdaVectors and xor_idx vec
  for (size_t ii = 0; ii < used_atom_vectors.size(); ii++)
    SortAtoms(used_atom_vectors[ii], lambda_ranges_vec[ii], xor_idx_vec[ii]);

  //if there is an XOR in the current FORMULA, erase every entry that doesn't fit the restriction, i.e. every entry,
  //in which two atoms with the same xor_idx value are identical
  if (has_xor) {
    //go through all the atom_vectors in used_atom_vectors
    for (int i = 0; i < (int)used_atom_vectors.size(); i++) {
      bool brk(false);

      //go through all the atom pairs (used_atom_vectors[i][ii], used_atom_vectors[i][jj])
      for (size_t ii = 0; ii < used_atom_vectors.size(); ii++) {
        for (size_t jj = ii + 1; jj < used_atom_vectors[i].size(); jj++) {
          //if two atoms in the current AtomVector are identical, and their xor_idx entries are identical and not equal to -1, the current AtomVector
          //violates the XOR restrictions and needs ot be erased from used_atom_vectors. The corresponding entries also need to be erased from
          //lambda_ranges_vec and xor_idx_vec
          if (used_atom_vectors[i][ii] == used_atom_vectors[i][jj] &&
              xor_idx_vec[i][ii] != -1 && xor_idx_vec[i][ii] == xor_idx_vec[i][jj]) {
        ...
        //don't keep looping over the current atom pairs, if the corresponding AtomVector is already erased
        if (brk)
          break;
      }
    }
  ...
  //go through all the atom_vectors in used_atom_vectors, and combine the entries that correspond to an identical atom type
  for (size_t i = 0; i < used_atom_vectors.size(); i++) {
    //go through all the pairs (used_atom_vectors[i][ii], used_atom_vectors[i][jj])
    for (size_t ii = 0; ii < used_atom_vectors[i].size(); ii++) {
      for (int jj = (int)ii + 1; jj < (int)used_atom_vectors[i].size(); jj++) {
        //if the atom entry ii is identical to the atom entry jj, combine the two by adding their lambda values and erasing the
        //jj entries in the lambda_ranges_vec and the used_atom_vectors
        if (used_atom_vectors[i][ii] == used_atom_vectors[i][jj]) {
          ...
          for (auto && lr : lambda_ranges_vec[i]) {
            ...
            lr[ii] += lr[jj];
            //lambda_ranges_vec[i][k].erase(lambda_ranges_vec[i][k].begin() + jj);
  ...
  //erase the atoms in the atom_vectors that have a lambda value of 0. If the thereby created AtomVector is already in used_atom_vectors,
  //simply erase it and add the additional lambda values, and otherwise, just add the newly created AtomVector with its lambdaValues
  size_t s = used_atom_vectors.size();

  for (size_t i = 0; i < s; i++) {
    bool erase(false);

    for (auto &&   lr = lambda_ranges_vec[i].begin(); lambda_ranges_vec[i].size() &&
         lr != lambda_ranges_vec[i].end(); lr++) {
      ...
      //search for zero entries in all entries of lambda_ranges_vec[i][jj] and erase the corresponding atom types
      ...
      //if current AtomVector was changed, check if it has to be added to used_atom_vectors, or if it's already in there
      if (zeroEntry) {
        auto lr2 = lr;
        ...
        //check if curr_atom_vec is found in used_atom_vectors and add the newly created LambdaVector
        for (size_t iii = 0; iii < used_atom_vectors.size(); iii++) {
          if (used_atom_vectors[iii] == curr_atom_vec) {
            found = true;
            //check if the corresponding LambdaVector that was just changed is already in the AtomVector's lamdaRanges
            //if not, add it to lambda_ranges_vec[iii]
            lambda_ranges_vec[iii].push_back(lambda_vec_tmp);
            break;
          }
        }

        //if curr_atom_vec is not in used_atom_vectors, add it toGether with its lambda vector
        if (!found) {
  ...
  //erase all atom_vectors that appear more than once in used_atom_vectors. Add the lambda_ranges of the deleted AtomVector to the ones of the one that's not deleted
  int resize = 0;
    ...
    for (int jj = ii + 1; jj < ((int)used_atom_vectors.size() - resize); jj++) {
      if (used_atom_vectors[ii] == used_atom_vectors[jj]) {
        lambda_ranges_vec[ii].insert(lambda_ranges_vec[ii].end(),
                                     lambda_ranges_vec[jj].begin(), lambda_ranges_vec[jj].end());
  ...
  for (size_t i = 0; i < used_atom_vectors.size(); i++) {
    for (auto && lr : lambda_ranges_vec[i]) {
      if (lr.size() != used_atom_vectors[i].size())
        throw std::runtime_error("size of lr is not equal to size of used_atom_vectors");
    }
  }
  ...
  //make sure that all entries in the lambda_ranges are unique for all the used_atom_vectors
  for (size_t i = 0; i < used_atom_vectors.size(); i++) {
    lambda_ranges_vec[i].sort();
    auto&& it = std::unique(lambda_ranges_vec[i].begin(), lambda_ranges_vec[i].end());
    lambda_ranges_vec[i].resize(std::distance(lambda_ranges_vec[i].begin(), it));
  }
...
READ THE ATOM TYPES GIVEN IN THE INPUT, IN ORDER TO DEFINE THE CURRENT FRAGMENT
 ->atom_vectors contains all the atom vectors that are allowed for the current fragment
 ->if none of the elements are alias_map, it contains just one atom vector
 ->otherwise, it will explicitly contain every possible combination of the regular and alias atom types
******************************************************************************/

void AddAtomTypes(std::string& frg, size_t& j, std::vector<int>& xor_idx,
                  std::vector<int>& and_idx, std::vector<AtomVector<combi_ff::Atom>>& atom_vectors,
                  const AliasMap& alias_map, const std::string& version) {
  std::string name("");

  //go through all the characters between the brackets [...] in frg
  while (j < frg.size()) {
    //case that atom name starts with a letter, or is an asterisk => add an entry of atom type * to all atom_vectors
    if (frg[j] == '*') {
      for (auto && a : atom_vectors)
        a.push_back(Atom("*"));
    ...
    //case that current atom is a united atom
      ...
      while (frg[++j] != '}' && isalpha(frg[j]))
        name += frg[j];

      //make sure that the united atom contains hydrogen
      if (name.back() != 'H')
        throw combi_ff::input_error("united atom notation only works for hydrogen at the moment, e.g. {CH3}");
      ...
      if (frg[j++] != '}')
        throw combi_ff::input_error("expected \'}\' in " + frg + " at position " +
                                    std::to_string(j - 1) + " but encountered " + std::string(1, frg[j - 1]));
...
void AddAtomType(std::string& name, const AliasMap& alias_map,
                 std::vector<AtomVector<combi_ff::Atom>>& atom_vectors, const std::string& version) {
  bool isAlias(false);
  ...
  if (alias != alias_map.end()) {
    ...
    atom_vectors_tmp.reserve(n * alias->second.GetContainedAtomTypesConst().size());
    ...
    for (size_t i = 0; i < n; i++) {
      ...
      for (auto  map : alias->second.GetContainedAtomTypesConst()) {
        ...
        if (map[j] == '{') {
          ...
          while (map[++j] != '}' && isalpha(map[j]))
            nam += map[j];

          if (nam.back() != 'H')
            throw combi_ff::input_error("united atom notation only works for hydrogen atm, e.g. {CH3}");
          ...
          //add a copy of the i-th AtomVector, and append the current atom to it
          atom_vectors_tmp.push_back(atom_vectors[i]);
          atom_vectors_tmp.back().push_back(Atom(nam.substr(0, nam.size() - 1)));
          atom_vectors_tmp.back().back().SetNumFixedHydrogens(n);

          if (map[j] != '}')
            throw combi_ff::input_error("expected \'}\' in " + map + " at position " +
                                        std::to_string(j) + " but encountered " + std::string(1, map[j]));
        }
...
*************************************************/
void CleanUpAtomVectorsFrag(std::vector<AtomVector<combi_ff::Atom>>&
                            atom_vectors, std::vector<int>& xor_idx, std::vector<int>& and_idx) {
  ...
  for (int i = 0; i < (int)atom_vectors.size(); i++) {
    ...
    for (size_t ii = 0; ii < a.size(); ii++) {
      //save XOR and AND idx, of first atom of the pair. save the atom name as well. just for convenience, since it's reused
      ...
      for (size_t jj = ii + 1; jj < a.size(); jj++) {
        ...
        if (currXOR != -1) {
          //if atom ii and jj have the same XOR index, but are of the same atom type => violation!
          if (currXOR == xor_idx[jj] && curr_atom_type == a[jj].GetElementSymbol())
            del = true;
        }

        //if current AtomVector was not deleted, and atom ii is an AND
        if (!del && currAND != -1) {
          //if atom ii and atom jj have the same AND index, but don't have the same atom type => violation!
          if (currAND == and_idx[jj] && curr_atom_type != a[jj].GetElementSymbol())
            del = true;
        }

...
MaxFillAlg::MaxFillAlg(enu::AdjacencyMatrix& A, const bool stereo, const size_t max_degree)
  : N(A.GetN()),
    stereo(stereo),
    i(0),
    j(0),
    ...
    perm_it(N),
    next_step_ptr(&MaxFillAlg::MaxFillAlg::ForwardStep) { //Set fw to true initially
  ...
  //e.g. for lambdaHat = [1, 3, 1, 4, 6], we have num_perms = [1, 3, 2, 1, 1, 4, 3, 2, 1, 6, 5, 4, 3, 2, 1]
  std::vector<size_t> num_perms(std::accumulate(lambda.begin(), lambda.end(), 0));
  size_t idx(0);
  ...
  //lambda_prime is used for the semicanonicity test, storing the refined partitions for each row
  ...
  for (auto && l : lambda_prime_prime)
    l.reserve(N);

  if (lambda[0] > 1)
    lambda_prime_prime[0].push_back(lambda[0] - 1);

  for (size_t k = 1; k < lambda.size(); k++)
    lambda_prime_prime[0].push_back(lambda[k]);

  for (size_t ii = 0; ii < N; ii++)
    num_perms_lambda_prime_prime[ii].reserve(N);
...
std::pair<int, int> MaxFillAlg::GetHat() {
  return {degree_vec[i] - (int)accumulated_row_matrix.GetElement(i, j), degree_vec[j] - (int)accumulated_column_matrix.GetElement(i, j)};
}
...
void MaxFillAlg::SetNextIndex() {
  //if we're at the end of a row, increase the row number and Set the column number s.t. we're one value off the diagonal
  if (j == N - 1) {
    i++;
    j = i + 1;
  }
...
void MaxFillAlg::SetPreviousIndex() {
  //if we're at a position that's one element off the diagonal, decrease the row number and Set j to the last column number
  if (j == i + 1) {
    i--;
    j = N - 1;
  }
...
void MaxFillAlg::ForwardStep() {
  ...
  //calculate the largest possible value that the current A can take for element (i,j) and the smallest possible value it has to take
  size_t x = std::min({l_hat, c_hat, m}); // note: l_hat, c_hat, m are all positive by definition!
  size_t min = std::max({0, l_hat - L.GetElement(i, j), c_hat - C.GetElement(i, j)}); // note: min is at least 0, i.e. non-negative

  if (num_perms_lambda_prime_prime[i][j - 1] > 1)
    x = std::min(A.GetElement(i, j - 1), x); //ensure semicanonicity

  //if the maximum possible entry is smaller than the minimum necessary entry -> backstep
  if (x < min)
    next_step_ptr = &MaxFillAlg::BackwardStep;

  //otherwise, accept the maximum possible entry as the new element at position (i,j)
  else {
    A.SetElement(i, j, x);
    accumulated_row_matrix.SetElement(i, j + 1, accumulated_row_matrix.GetElement(i, j) + x);
    accumulated_column_matrix.SetElement(i + 1, j, accumulated_column_matrix.GetElement(i, j) + x);

    //if we're at the end of a row, calculate the next lambdas
    if (j == N - 1) {
      CalculateNextLambdas();
      //off-diagonal element
      accumulated_row_matrix.SetElement(i + 1, i + 2,
                                        accumulated_column_matrix.GetElement(i + 1, i + 1));

      //if all the rows are filled OR all the rows of the current atom type are filled -> test for canonicity
...
void MaxFillAlg::BackwardStep() {
  ...
  if (j == 1) {
    ...
    if ((x_minus_one >= 0) && (l_hat - x_minus_one <= L.GetElement(i, j)) &&
      ...
      accumulated_row_matrix.SetElement(i, j + 1, accumulated_row_matrix.GetElement(i, j + 1) - 1);
      accumulated_column_matrix.SetElement(i + 1, j, accumulated_column_matrix.GetElement(i + 1, j) - 1);
      next_step_ptr = &MaxFillAlg::ForwardStep;
...
void MaxFillAlg::CalculateNextLambdas() {
  LambdaVector& lambda_prime_prime_prev = lambda_prime_prime[i]; //lambda^{i-1}''
  ...
  //lambda_prime_prime_next.assign(lambda_prime_prime_next.end(), lambda_prime_cur.begin() + 1, lambda_prime_cur.end());
  ...
  for (size_t ii = 0; ii < lambda_prime_prime_next.size(); ii++) {
    for (size_t jj = 0; jj < lambda_prime_prime_next[ii]; jj++)
      num_perms_lambda_prime_prime[i + 1][idx++] = (lambda_prime_prime_next[ii] - jj);
  }
}
...
bool MaxFillAlg::IsCanonical() {
  ...
  while (perm_it.GetNextPermutation()) {
    ...
    //the block that is currently tested is not affected by perm, and the next blocks also won't be
    //-> add to u_next (for automorphism group for stereo), and continue from the next permutation for smallestDiffIdx
    ...
    } else if (!FindStabilizer(*permuted_indices, u_next, highest_permuted_idx)) {
  ...
  //these permutations only swap cols in A(r) and only within the lambda z prime partitions where the entries are the same within the same blocks, but not any rows
  for (size_t ii = /*z+1*/i + 1; ii < u_next.size(); ii++)
    u_next[ii].assign(row_stabilizer_rep[0][ii].begin(),
...
bool MaxFillAlg::FindStabilizer(const std::vector<size_t>& permuted_indices,
  ...
  if (A.SmallerThanPermuted(i, permuted_indices, i_pos, j_pos, i_pos_perm, j_pos_perm,
                            i0, j0, diff, perm_it.GetSmallestDiffIndex(), highest_permuted_idx)) {
  ...
  if (!diff) {
    //automorphism found
    u_next[perm_it.GetSmallestDiffIndex()].push_back(*perm_it.GetCombinedPermutation());
    perm_it.SetCurrentIndexToSmallestDiffIndex();

  } else if (j_pos < perm_it.GetCurrentIndex()) {
    //skip if position where the first difference was found is smaller than what is affected by the next permutation
...
void MaxFillAlg::DetermineNewIJ(const size_t i0, const size_t j0,
                                const size_t i_pos, const size_t j_pos, const size_t i_pos_perm,
  ...
  i = (maximum).first;
  j = (maximum).second;
  SetNextIndex(); //necessary because in the next bw step, we will decrease (i,j)
}

const RepresentationSystem& MaxFillAlg::GetUAutomorph() const {
  return row_stabilizer_rep.back();
}
const RepresentationSystem& MaxFillAlg::GetUId() const {
  return id;
}
...
RepresentationSystem& MaxFillAlg::GetUId_non_const() {
  return id;
}
...
/*********************************************************************************
CALCULATE THE SUPPLEMENTARY MATRICES M, L, AND C FOR THE MAXIMUM FILLING ALGORITHM
- the element (i,j) of M contains the maximum possible bond degree between atoms i and j
- the element (i,j) of L corresponds to the maximum possible row capacity after (i,j)
- the element (i,j) of C corresponds to the maximum possible column capacity after (i,j)
*********************************************************************************/
void MaxFillAlg::GetMLC(const size_t max_degree) {
  ...
  // - if the degree of atoms i and j is the same, Set the value to the minimum of max_degree and deg -1
  // - otherwise, Set the value to the minimum of max_degree and the minimal value between deg(atom i) and deg(atom j)
  else {
    for (size_t ii = 0; ii < N; ii++) {
  ...
  for (size_t ii = 0; ii < N; ii++) {
    for (size_t jj = ii + 1; jj < N; jj++)
      L.SetElement(ii, jj, std::min(degree_vec[ii], M.AccumulateRow(ii, jj + 1, N)));
  }

  //fill C by going through indices (i,j)
  for (size_t ii = 0; ii < N; ii++) {
    for (size_t jj = ii + 1; jj < N; jj++)
      C.SetElement(ii, jj, std::min(degree_vec[jj], M.AccumulateColumn(ii + 1, N, jj)));
  }
}

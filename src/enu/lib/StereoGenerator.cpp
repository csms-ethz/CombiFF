...
StereoGenerator::StereoGenerator(const enu::AdjacencyMatrix& A,
  ...
  : A(A),
    ...
    stereo_smiles(std::vector<std::tuple<std::string, int, std::pair<int, int>>>(0)) {
  for (size_t i = 0; i < visited_indices.size(); i++)
    visited_idx[visited_indices[i]] = i; //visited_idx is inverse of visited_indices

  if (n_rings)
    is_in_cycle = A.IsInCycle();
}

std::vector<std::tuple<std::string, int, std::pair<int, int>>>& StereoGenerator::GetStereoSmiles() {
...
void StereoGenerator::GenerateStereoSmiles() {
  ...
  if (!(true_stereo_idx.size() || true_cis_trans_stereo_idx.size()))
    return;

  FindPotentialParaStereoCenters();
  std::vector<Configuration> valid_configurations;
  valid_configurations.reserve(true_stereo_idx.size() * true_cis_trans_stereo_idx.size());
...
void StereoGenerator::FindPotentialStereoCenters() {
  ...
  for (size_t i = 0; i < N; i++) {
    ...
    if (n_neighbors == 4 ||
        (n_neighbors == 3 && (a->GetNumFixedHydrogens() + a->GetNumHydrogens() == 1)))
      TestIfPotentialTetrahedralStereoCenter(*a, i);

    //test if an atom is a potential cis trans stereo center
    else if ((n_neighbors == 3 ||
              (n_neighbors == 2 && (a->GetNumFixedHydrogens() + a->GetNumHydrogens() == 1))) && !is_in_cycle[i])
      TestIfPotentialCisTransStereoCenter(*a, i);

    //atoms are ordered by bond degree, so as soon as an atom in the atom vec has a bond degree of less
    //than 2, all the following atoms will also have a bond degree of less than 2 and we can stop
    else if (n_neighbors < 2)
      break;
  }
}

void StereoGenerator::TestIfPotentialTetrahedralStereoCenter(const Atom& a, const size_t idx) {
  ...
  for (size_t i = 0 ; i < a.GetNeighbours().size(); i++) {
    ...
    for (size_t j = i + 1; j < a.GetNeighbours().size(); j++) {
      ...
      if (nbr1->GetUnitedAtomSymbol() == nbr2->GetUnitedAtomSymbol() &&
          ...
          && (nbr1->GetNumFixedHydrogens() + nbr1->GetNumHydrogens() == nbr2->GetNumFixedHydrogens() +
              nbr2->GetNumHydrogens())) {
...
void StereoGenerator::TestIfPotentialCisTransStereoCenter(const Atom& a, const size_t idx) {
  ...
  for (auto && nbr : a.GetNeighbours()) {
    if (A.GetElement(idx, nbr) == 2) {
      ...
      double_bond_partner[idx] = (int)nbr;
               ...
               (single_bond_partner[idx] == -1 || visited_idx[nbr] < visited_idx[single_bond_partner[idx]]))
...
void StereoGenerator::FindTrueStereoCenters() {
  ...
  while (perm_it.GetNextPermutation() &&
    ...
    for (size_t i = 0; i < potential_stereo_idx.size(); i++) {
      ...
      // only look at the nbr permutations if the permutation leaves the current atom identical
    ...
    for (size_t i = 0; i < potential_cis_trans_stereo_idx.size(); i++) {
      ...
      if ((*permuted_indices)[idx] == idx) {
        if (NeighborsArePermuted(*a, *permuted_indices)) {
          ...
          potential_cis_trans_stereo_idx.erase(potential_cis_trans_stereo_idx.begin() + i);
  ...
  /*
  remove from potentical cis trans stereo and true stereo, if the neighbour is not a true or potential  cis trans stereo
  */
  ...
  for (size_t i = 0; i < true_cis_trans_stereo_idx.size(); i++) {
    if (double_bond_partner[true_cis_trans_stereo_idx[i]] > (int)true_cis_trans_stereo_idx[i]) {
      CisTransPair ct(true_cis_trans_stereo_idx[i], double_bond_partner[true_cis_trans_stereo_idx[i]],
                      single_bond_partner[true_cis_trans_stereo_idx[i]],
                      single_bond_partner[double_bond_partner[true_cis_trans_stereo_idx[i]]]);
  ...
  for (size_t i = 0; i < true_cis_trans_pairs.size(); i++) {
    for (size_t j = i + 1; j < true_cis_trans_pairs.size(); j++) {
      if (visited_idx[true_cis_trans_pairs[i].idx1] > visited_idx[true_cis_trans_pairs[j].idx1])
        std::swap(true_cis_trans_pairs[i], true_cis_trans_pairs[j]);

      else if (visited_idx[true_cis_trans_pairs[i].idx1] == visited_idx[true_cis_trans_pairs[j].idx1] &&
               visited_idx[true_cis_trans_pairs[i].idx2] > visited_idx[true_cis_trans_pairs[j].idx2])
        std::swap(true_cis_trans_pairs[i], true_cis_trans_pairs[j]);
    }
  }
}
...
void StereoGenerator::FindPotentialParaStereoCenters() {
  if (potential_para_stereo_idx.size() || potential_para_cis_trans_stereo_idx.size()) {
    ...
    while (perm_it.GetNextPermutation()) {
      ...
      //if the permutation swaps two true stereocenters, it might not be an automorphism of the adj matrix
      //when the two stereocenters have a different configuration
      // (note if one of them , e.g. the first, is a true s.c., then both are, i.e. if true_stereo[pp.first] is true it must hold that trueStereo[pp.second] is also true, otherwise they couldn't be swapped!
      for (auto && pp : *stab) {
    ...
    while (perm_it_local.GetNextPermutation() && (potential_para_stereo_idx.size() ||
      ...
      for (size_t i = 0; i < potential_para_stereo_idx.size(); i++) {
        ...
        if ((*permuted_indices)[idx] == idx) {
          if (NeighborsArePermuted(A.GetAtomVector()[idx], *permuted_indices)) {
            potential_para_stereo_idx.erase(potential_para_stereo_idx.begin() + i);
            i--;
          }
        }
      }
      ...
      for (size_t i = 0; i < potential_para_cis_trans_stereo_idx.size(); i++) {
        ...
        if ((*permuted_indices)[idx] == idx) {
          if (NeighborsArePermuted(A.GetAtomVector()[idx], *permuted_indices)) {
            potential_para_cis_trans_stereo_idx.erase(potential_para_cis_trans_stereo_idx.begin() + i);
            i--;
          }
        }
      }
    ...
    if (potential_para_stereo_idx.size() || potential_para_cis_trans_stereo_idx.size()) {
      ...
      //remove CT para stereo centers that are not connected to another para or true CT stereocenter
      ...
      } while (removed_para_ct_stereo && potential_para_cis_trans_stereo_idx.size());
      ...
      while (perm_it.GetNextPermutation()) {
        ...
        for (size_t i = 0; i < true_stereo_idx.size(); i++) {
          ...
          if ((*permuted_indices)[idx] > idx &&
              std::find(identical_stereo_center_pairs.begin(), identical_stereo_center_pairs.end(), Permutation({idx, (*permuted_indices)[idx]}))
              == identical_stereo_center_pairs.end()) {
            identical_stereo_center_pairs.push_back(Permutation({idx, (*permuted_indices)[idx]}));
          }
        }
        ...
        for (size_t i = 0; i < true_cis_trans_stereo_idx.size(); i++) {
          ...
          if ((*permuted_indices)[idx] > idx &&
              std::find(identical_ct_stereo_center_pairs.begin(), identical_ct_stereo_center_pairs.end(),
                        Permutation({idx, (*permuted_indices)[idx]}))
              == identical_ct_stereo_center_pairs.end()) {
            identical_ct_stereo_center_pairs.push_back(Permutation({idx, (*permuted_indices)[idx]}));
          }
        }
      }
      ...
      for (auto && is : identical_stereo_center_pairs) {
        if (position_in_identical_stereo_centers[is.first] != -1) {
          identical_stereo_centers[position_in_identical_stereo_centers[is.first]].push_back(is.first);
          identical_stereo_centers[position_in_identical_stereo_centers[is.first]].push_back(is.second);
          position_in_identical_stereo_centers[is.second] = position_in_identical_stereo_centers[is.first];

        } else if (position_in_identical_stereo_centers[is.second] != -1) {
          identical_stereo_centers[position_in_identical_stereo_centers[is.second]].push_back(is.first);
          identical_stereo_centers[position_in_identical_stereo_centers[is.second]].push_back(is.second);
          position_in_identical_stereo_centers[is.first] = position_in_identical_stereo_centers[is.second];

        } else {
          identical_stereo_centers.emplace_back(std::vector<size_t> {is.first, is.second});
          position_in_identical_stereo_centers[is.first] = (int)identical_stereo_centers.size() - 1;
          position_in_identical_stereo_centers[is.second] = (int)identical_stereo_centers.size() - 1;
        }
      }

      for (auto && is : identical_ct_stereo_center_pairs) {
        if (position_in_identical_ct_stereo_centers[is.first] != -1) {
          identical_ct_stereo_centers[position_in_identical_ct_stereo_centers[is.first]].push_back(is.first);
          identical_ct_stereo_centers[position_in_identical_ct_stereo_centers[is.first]].push_back(is.second);
          ...
          identical_ct_stereo_centers[position_in_identical_ct_stereo_centers[is.second]].push_back(is.first);
          identical_ct_stereo_centers[position_in_identical_ct_stereo_centers[is.second]].push_back(
            is.second);
          position_in_identical_ct_stereo_centers[is.first] =
            position_in_identical_ct_stereo_centers[is.second];

        } else {
          identical_ct_stereo_centers.emplace_back(std::vector<size_t> {is.first, is.second});
          position_in_identical_ct_stereo_centers[is.first] = (int)identical_ct_stereo_centers.size() - 1;
          position_in_identical_ct_stereo_centers[is.second] = (int)identical_ct_stereo_centers.size() - 1;
        }
      }
        ...
        for (size_t i = 0; i < id.size(); i++) {
          for (size_t j = i + 1; j < id.size(); j++) {
            ...
            if (minimal_path.size() % 2 != 0)
              symmetry_center_atoms.push_back(minimal_path[minimal_path.size() / 2 ]);
          }
        }
      }
      ...
      auto&& it1 = std::unique(symmetry_center_atoms.begin(), symmetry_center_atoms.end());
      symmetry_center_atoms.resize(std::distance(symmetry_center_atoms.begin(), it1));
      std::vector<bool> is_symmetry_atom(N, false);

      for (auto && idx : symmetry_center_atoms)
        is_symmetry_atom[idx] = true;
      ...
      for (size_t i = 0; i < N; i++) {
        if (!is_in_cycle[i]) {
          ...
          } else if (para_cis_trans_stereo[i] && is_symmetry_atom[double_bond_partner[i]] &&
      ...
      for (size_t i = 0; i < potential_para_cis_trans_stereo_idx.size(); i++) {
        if (double_bond_partner[potential_para_cis_trans_stereo_idx[i]] > (int)
            potential_para_cis_trans_stereo_idx[i]) {
          if (visited_idx[potential_para_cis_trans_stereo_idx[i]] <
              visited_idx[double_bond_partner[potential_para_cis_trans_stereo_idx[i]]]) {
            para_cis_trans_pairs.emplace_back(potential_para_cis_trans_stereo_idx[i],
                                              double_bond_partner[potential_para_cis_trans_stereo_idx[i]],
                                              single_bond_partner[potential_para_cis_trans_stereo_idx[i]],
                                              single_bond_partner[double_bond_partner[potential_para_cis_trans_stereo_idx[i]]]);

          } else {
            para_cis_trans_pairs.emplace_back(double_bond_partner[potential_para_cis_trans_stereo_idx[i]],
                                              potential_para_cis_trans_stereo_idx[i],
                                              single_bond_partner[double_bond_partner[potential_para_cis_trans_stereo_idx[i]]],
                                              single_bond_partner[potential_para_cis_trans_stereo_idx[i]]);
          }
      ...
      for (size_t i = 0; i < para_cis_trans_pairs.size(); i++) {
        for (size_t j = i + 1; j < para_cis_trans_pairs.size(); j++) {
          if (visited_idx[para_cis_trans_pairs[i].idx1] > visited_idx[para_cis_trans_pairs[j].idx1])
            std::swap(para_cis_trans_pairs[i], para_cis_trans_pairs[j]);

          else if (visited_idx[para_cis_trans_pairs[i].idx1] == visited_idx[para_cis_trans_pairs[j].idx1] &&
                   visited_idx[para_cis_trans_pairs[i].idx2] > visited_idx[para_cis_trans_pairs[j].idx2])
            std::swap(para_cis_trans_pairs[i], para_cis_trans_pairs[j]);
        }
      }
    }
...
void StereoGenerator::FindValidConfigurations(std::vector<Configuration>& valid_configurations) {
  ...
  if ((valid_configurations_tetra.size() || valid_configurations_ct.size()) &&
      (potential_para_stereo_idx.size() || potential_para_cis_trans_stereo_idx.size()))
    FindValidParaConfigurations(valid_configurations);
}


void StereoGenerator::FindValidTrueTetrahedralConfigurations(std::vector<Config>&
                                                             valid_configurations_tetra) {
  ...
  do {
    ...
    while (perm_it.GetNextPermutation()) {
      ...
      for (size_t j = 0; j < true_stereo_idx.size(); j++) {
        ...
        if (idx != idx_permuted) {
          NeighborOrder(idx, idx_permuted, *permuted_indices, nbrs_original_order, nbrs_permuted_order);
    ...
    if (!ignore)
      valid_configurations_tetra.push_back(conf);
  } while (conf != all_true);
}


void StereoGenerator::FindValidTrueCTConfigurations(std::vector<Config>&
                                                    valid_configurations_ct) {
  ...
  do {
    ...
    while (perm_it.GetNextPermutation()) {
      ...
      for (size_t j = 0; j < true_cis_trans_pairs.size(); j++) {
        ...
        if (idx1 != idx_permuted1 || idx2 != idx_permuted2) {
          // find the permuted cistrans pair of (idx1,idx2), i.e. (idx_permuted1, idx_permuted2)
          for (size_t ii = 0; ii < true_cis_trans_pairs.size(); ii++) {
            // case that idx1 is swapped with idx1 of the ii-th true_cis_trans_pairs
            ...
            // case that idx1 is swapped with idx2 of the ii-th true_cis_trans_pairs
    ...
    if (!ignore)
      valid_configurations_ct.push_back(configuration_ct);
  } while (configuration_ct != all_true_ct);
}

void StereoGenerator::AddValidTrueConfigurations(std::vector<Configuration>& valid_configurations,
  ...
  if (valid_configurations_tetra.size()) {
    if (valid_configurations_ct.size()) {
      valid_configurations.resize(valid_configurations_tetra.size()*valid_configurations_ct.size());
      size_t idx(0);

      for (size_t i = 0; i < valid_configurations_tetra.size(); i++) {
        for (size_t j = 0; j < valid_configurations_ct.size(); j++)
          valid_configurations[idx++] = (Configuration(valid_configurations_tetra[i],
                                                       valid_configurations_ct[j]));
      }
      ...
      for (size_t i = 0; i < valid_configurations_tetra.size(); i++)
        valid_configurations[i] = (Configuration(valid_configurations_tetra[i], Config(0)));
    }
    ...
    for (size_t j = 0; j < valid_configurations_ct.size(); j++)
      valid_configurations[j] = (Configuration(Config(0), valid_configurations_ct[j]));
  }
}

void StereoGenerator::FindValidParaConfigurations(std::vector<Configuration>&
                                                  valid_configurations) {
  std::vector<int> position_in_potential_para_stereo_idx(N, -1);

  for (size_t i = 0; i < potential_para_stereo_idx.size(); i++)
    position_in_potential_para_stereo_idx[potential_para_stereo_idx[i]] = (int)i;
  ...
  for (size_t k = 0; k < vcs; k++) {
    ...
    //go through permutations to find stabilizers, taking into account the configurations of the true stereocenters
    while (perm_it.GetNextPermutation()) {
      bool add(true);
      permuted_indices = perm_it.GetPermutedIndices();
      ...
      for (size_t i = 0; i < true_stereo_idx.size() && add; i++) {
        ...
        if (idx != idx_permuted) {
          ...
          if (NumPerm(nbrs_original_order, nbrs_permuted_order) % 2 == 0) {
            if (vc_tet[i] != vc_tet[position_in_true_stereo_idx[(*permuted_indices)[idx]]]) {
              add = false;
              break;
            }
      ...
      if (add) {
        local_stabilizers[perm_it.GetSmallestDiffIndex()].push_back(*(perm_it.GetCombinedPermutation()));
        perm_it.SetCurrentIndexToSmallestDiffIndex();
      }
    }
    ...
    do {
      ...
      do {
          ...
          while (perm_it_local.GetNextPermutation()) {
              ...
              if (idx != idx_permuted) {
                ...
                if (NumPerm(nbrs_original_order, nbrs_permuted_order) % 2 == 0) {
                  if (para_configuration[i] !=
                      para_configuration[position_in_potential_para_stereo_idx[(*permuted_indices)[idx]]]) {
                    skip = true;
                    break;
                  }
                  ...
                  if (para_cis_trans_pairs[ii].idx1 == idx_permuted1 &&
                    ...
                    if (para_cis_trans_pairs[ii].nbr1 == nbr_permuted1) {
                      if (para_cis_trans_pairs[ii].nbr2 == nbr_permuted2) {
                        if (para_configuration_ct[i] != para_configuration_ct[ii]) {
                          skip = true;
                          break;
                        }
            ...
            if (!skip) {
              for (size_t i = 0; i < potential_para_stereo_idx.size(); i++) {
                ...
                if ((*permuted_indices)[idx] == idx) {
                  if (NeighborsArePermuted(*a, *permuted_indices) && para_configuration[i] != -1) {
        ...
        if (para_configuration != all_not_para || para_configuration_ct != all_not_para_ct) {
          ...
          for (size_t i = 0; i < potential_para_stereo_idx.size(); i++)
            configuration_all[potential_para_stereo_idx[i]] = para_configuration[i];
          ...
          while (perm_it_local.GetNextPermutation()) {
            ...
            for (size_t j = 0; j < potential_para_stereo_idx.size(); j++) {
              if (para_configuration[j] == -1)
                ...
                if (NumPerm(nbrs_original_order, nbrs_permuted_order) % 2 == 0)
                  configuration_permuted[j] = configuration_all[(*permuted_indices)[potential_para_stereo_idx[j]]];
                ...
                if (idx1 != idx_permuted1 || idx2 != idx_permuted2) {
                  for (size_t ii = 0; ii < para_cis_trans_pairs.size(); ii++) {
                    if (para_cis_trans_pairs[ii].idx1 == idx_permuted1 &&
                      ...
                      if (para_cis_trans_pairs[ii].nbr1 == nbr_permuted1) {
                        if (para_cis_trans_pairs[ii].nbr2 == nbr_permuted2)
                          configuration_permuted_ct[j] = para_configuration_ct[ii];
            ...
            if (IsSmaller(configuration_permuted_ct, para_configuration_ct_orig)) {
              ignore = true;
              break;
            }
          }
          ...
          if (!ignore) {
            ...
            new_configuration.reserve(vc_tet.size() + para_configuration.size());
            new_configuration.insert(new_configuration.end(), vc_tet.begin(), vc_tet.end());
            ...
            new_configuration_ct.reserve(vc_ct.size() + para_configuration_ct.size());
            new_configuration_ct.insert(new_configuration_ct.end(), vc_ct.begin(), vc_ct.end());
            new_configuration_ct.insert(new_configuration_ct.end(),
                                        para_configuration_ct.begin(),
                                        para_configuration_ct.end());
            valid_configurations.push_back(Configuration(new_configuration, new_configuration_ct));
          }
        }
    ...
    if (found_new_configuration) {
      ...
      valid_configurations[k].valid_config_tetra.insert(valid_configurations[k].valid_config_tetra.end(),
                                                        all_not_para.begin(),
                                                        all_not_para.end());
      valid_configurations[k].valid_config_ct.insert(valid_configurations[k].valid_config_ct.end(),
                                                     all_not_para_ct.begin(),
                                                     all_not_para_ct.end());
    }
  }
}

void StereoGenerator::WriteStereoSmiles(std::vector<Configuration>& valid_configurations) {
  ...
  all_cis_trans_pairs.reserve(true_cis_trans_pairs.size() + para_cis_trans_pairs.size());
  ...
  for (size_t i = 0; i < all_cis_trans_pairs.size(); i++) {
    for (size_t j = i + 1; j < all_cis_trans_pairs.size(); j++) {
      if (visited_idx[all_cis_trans_pairs[i].idx1] > visited_idx[all_cis_trans_pairs[j].idx1]) {
        for (auto && valid_conf : valid_configurations)
          std::swap(valid_conf.valid_config_ct[i], valid_conf.valid_config_ct[j]);

        std::swap(all_cis_trans_pairs[i], all_cis_trans_pairs[j]);

      } else if (visited_idx[all_cis_trans_pairs[i].idx1] == visited_idx[all_cis_trans_pairs[j].idx1] &&
                 visited_idx[all_cis_trans_pairs[i].idx2] > visited_idx[all_cis_trans_pairs[j].idx2]) {
        for (auto && valid_conf : valid_configurations)
          std::swap(valid_conf.valid_config_ct[i], valid_conf.valid_config_ct[j]);
  ...
  for (size_t pos = 0; pos < valid_configurations.size(); pos++) {
    ...
    vc.assign(valid_conf.valid_config_tetra.begin(), valid_conf.valid_config_tetra.end());
    std::fill(vc_flipped.begin(), vc_flipped.end(), -1);

    for (size_t i = 0; i < smiles_blocks_local.size(); i++) {
      //only Reset the element_name, as it is the only thing that's potentially changed!
    ...
    for (size_t i = 0; i < potential_para_stereo_idx.size(); i++) {
      if (vc[i + true_stereo_idx.size()] != -1) {
        ...
        smiles_blocks_local[potential_para_stereo_idx[i]].element_name.insert(0, "[");

        if (vc[i + true_stereo_idx.size()]) {
          vc_flipped[i + true_stereo_idx.size()] = 0;

          if (A.GetAtomVector()[potential_para_stereo_idx[i]].GetNumHydrogens() +
              A.GetAtomVector()[potential_para_stereo_idx[i]].GetNumFixedHydrogens())
            smiles_blocks_local[potential_para_stereo_idx[i]].element_name += "@H]";
    ...
    for (size_t i = 0; i < all_cis_trans_pairs.size(); i++) {
      if (vc[i] != -1) {
        ...
        else if (smiles_blocks_local[all_cis_trans_pairs[i].idx1].element_name[0] != '/')
          smiles_blocks_local[all_cis_trans_pairs[i].idx1].element_name.insert(0, "/");

        // case trans
        if (!vc[i]) {
          if (first_bond_down)
            smiles_blocks_local[all_cis_trans_pairs[i].nbr2].element_name.insert(0, "\\");
    ...
    for (size_t i = 0; i < valid_configurations.size(); i++) {
      if (valid_configurations[i].valid_config_tetra == vc_flipped && pos != i &&
          valid_configurations[i].valid_config_ct == valid_conf.valid_config_ct) {
        enantiomer = (int)i;
        break;
      }
    }
    ...
    for (auto && smi : smiles_blocks_local)
      length += smi.size();
    ...
    stereo_smiles.emplace_back(std::tuple<std::string, int, std::pair<int, int>> {smiles, enantiomer, std::pair<int, int>(num_stereo_centers_local, num_ct_bonds_local)});
  }
}
...
void StereoGenerator::FindShortestPath(std::vector<size_t>& minimal_path, size_t i,
                                       size_t j) const {
...
size_t StereoGenerator::NumPerm(const std::vector<size_t>& original,
  ...
  //not clear yet how many swaps -> e.g. 1,0,2,3 to 3,2,0,1 are only 2, but to 0,2,3,1 are 3
  case (4): {
    for (size_t i = 0; i < 4; i++) {
      if (permutated[i] == original[0]) {
        //if zeroth value of original is swapped with the same number in both vectors, even permutations (2 swaps), e.g. 1,0,2,3 and 3,2,0,1
        //note: doesn't necessarily have to be element zero, can also be any of the other numbers
        if (permutated[0] == original[i]) {
          //std::cout << 2 << std::endl;
          return 2;
        }

        //if zeroth value of permutated is swapped with different number, odd permutations (3 swaps), e.g. 1,0,2,3 and 0,2,3,1
        //note: doesn't necessarily have to be element zero, can also be any of the other numbers
  ...
  default:
    throw std::runtime_error("diff should be 0, 2, 3, or 4, but it is " + std::to_string(diff));
  }
}
...
void StereoGenerator::NeighborOrder(const size_t idx, const size_t idx_permuted,
                                    const std::vector<size_t>& permuted_indices,
                                    std::vector<size_t>& nbrs_original_order, std::vector<size_t>& nbrs_permuted_order) const {
  nbrs_original_order.resize(1);
  nbrs_original_order[0] = coming_from[idx];
  nbrs_original_order.insert(nbrs_original_order.end(), ring_connections[idx].begin(),
                             ring_connections[idx].end());
  nbrs_original_order.insert(nbrs_original_order.end(), going_to[idx].begin(), going_to[idx].end());
  nbrs_permuted_order.resize(1);
  nbrs_permuted_order[0] = std::distance(permuted_indices.begin(), std::find(permuted_indices.begin(),
                                                                             permuted_indices.end(), coming_from[idx_permuted]));

  for (const auto& rc : ring_connections[idx_permuted])
    nbrs_permuted_order.push_back(std::distance(permuted_indices.begin(),
...
size_t StereoGenerator::GetNumCTBonds() const {
  return num_ct_bonds;
}

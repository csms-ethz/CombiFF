...
IOFileProperties::IOFileProperties()
  ...
  for (size_t f = 0; f < num_input_files; f++) {
    auto type = possible_input_file_properties.find(static_cast<InputFileType>(f));

    if (type == possible_input_file_properties.end())
      throw std::logic_error("input file type " + std::to_string(f) + " not found\n");
...
//read the input options and use them to define the enum specifications and IOFiles
void InputOutput::ReadInputArguments() {
  ...
  for (size_t i = 0; i  < arguments.size(); i++)
    GetNextInputOption(i);
}

//read the keywords in the arguments vector and perform the corresponding action
void InputOutput::GetNextInputOption(size_t& i) {
  //arg is the current argument
  std::string arg = arguments[i];

  if (arg.front() != '-')
    throw combi_ff::input_error("expected a keyword starting with \'-\', but got \'" + arg +
                                "\'. To view input options, run the program with command line argument \'help\'");
  ...
  if (arg == "-input")
  ...
  else if (possible_ranges.find(arg.substr(1, arg.size() - 1)) != possible_ranges.end())
    AddRestriction(i, possible_ranges.find(arg.substr(1, arg.size() - 1))->second);
    ...
    throw combi_ff::input_error("unrecognized keyword \'" + arg +
                                "\' in input. To view input options, run the program with command line argument \'help\'");
}
...
// ->for each atom type, first read the atom type name, and then read the different lambdas
void InputOutput::AddAtoms(size_t& i) {
  ...
  while (j < formula.size()) {
    ...
    if (formula[j] == '{')
      AddUnitedAtom(j, formula, used_atoms);
    ...
    //if an atom name starts with a letter, read the name
      ...
      throw combi_ff::input_error("format error in formula: expected an alphabetical character at position "
  ...
  //make sure that all the atoms in the final used_atoms vector are unique. if necessary, combine two entries in used_atoms/lambda_ranges
  for (j = 0; j < used_atoms.size(); j++) {
    for (int k = (int)j + 1; k < (int)used_atoms.size(); k++) {
      if (used_atoms[j] == used_atoms[k]) {
        //std::cout << "combining atom " << j << " and " << k << " as they have the same type\n";
  ...
  //sort Atoms in decreasing order of degrees, and alphabetically, and make sure that all the resulting vectors in lambda_ranges are unique
  ...
  //add used_atoms and lambda_ranges to the usedAtomVectors and the lambda_rangesVec
  enum_spec.used_atom_vectors.push_back(used_atoms);
  enum_spec.lambda_ranges_vec.push_back(lambda_ranges);
}

//adds files of inputType to tempInputFile and opens the tempInputFile in inputFile
void InputOutput::ReadFileNames(size_t& i, const InputFileType inputType) {
  const std::string& name = io_file_properties.input_file_name_building_blocks[inputType];
  std::string line;
...
void InputOutput::AddFamilies(size_t& i) {
  ...
  auto&& it = std::unique(enum_spec.used_families.begin(), enum_spec.used_families.end());
  ...
  if (it != enum_spec.used_families.end()) {
    ...
    throw combi_ff::input_error("Please make sure not to list a family twice in the -families argument");
  }
}

//read in the output file name for the direct enumeration after the -output keyword
void InputOutput::AddOutputFile(size_t& i) {
  if (i + 1 >= arguments.size())
    throw combi_ff::input_error("expected argument after -output");

  if (io_file_properties.output_file_name == "" || io_file_properties.output_file_name == IOFileProperties::output_file_name_default)
...
void InputOutput::AddMaxDegree(size_t& i) {
  ...
  try {
    ...
    throw combi_ff::input_error("expected integer argument after -maxDeg, but encountered " +
                                arguments[i] + " . " + ia.what());
  }
}
//read in the range of a rangedProperty
void InputOutput::AddRestriction(size_t& i, const size_t position_in_range_vec) {
...
void InputOutput::AddAtom(size_t& j, std::string& formula, AtomVector<combi_ff::Atom>& used_atoms) {
  std::string nam;
...
//read the atom type of a united atom, Set the number of fixed hydrogens and add the united atom to used_atoms
void InputOutput::AddUnitedAtom(size_t& j, std::string& formula,
  ...
  if (nam.back() != 'H')
    throw combi_ff::input_error("Error: united atom notation only works for hydrogen, use e.g. {CH3}");

  //add an atom with typeName "nam" to the used_atoms vector, leaving out the last letter corresponding to the hydrogen atom
  ...
  //after the name and number have been read, the united atom formula needs to be closed with a '}'
  //advance j to the next position after the '}'
  if (formula[j++] != '}')
...
const EnumSpecifications& InputOutput::GetEnumSpec() const {
  return enum_spec;
}
...
void InputOutput::PrintInputOptions() {
  std::cout << "input options are:\n\n"
            << "-input: used to specify an input file that can contain one or several of the following arguments.\n"
            << "   - Note that these arguments could also be used directly on the command line\n"
            << "   - lines starting with a # are considered comments and are ignored\n\n"
            << "-output_directory: used to specify in which directory the '.fie' files should be saved\n\n"
            << "-output: used to specify the file name of the '.fie' file that is created for the formulas given by -atoms\n\n"
            << "-atoms: used to specify a molecular formula. The following formats can be used to specify the amounts and types of the atoms:\n"
            ...
            << "   - the notation of [n-m] and [n,m,...] can be combined at will, e.g. [1-3,4,10-8, 7]\n"
            << "   - X*, in order to specify any amount of atom type X (in practice at the moment simply corresponds to range of 0-100)\n"
            << "   - X>n or X>=n, where X is an atom type and n is a number, in order to specify that there should at least n+1, or n, atoms of type X\n"
            << "   - X<n or X<=n, where X is an atom type and n is a number, in order to specify that there should be at most n-1, or n, atoms of type X\n"
            << "   - {XHn} can be used instead of X for all of the above options in order to specify a united atom, where X is an atom type and n is a number,\n     specifying the amount of hydrogen atoms in the united atom\n"
            << "   - the -atoms keyword can be used several times, in order to specify different molecules\n"
            << "   - example: -atoms C10H12Cl>1 F[2,3,5-4] -atoms C10 {CH3}4 H*\n\n"
            << "-max_bond_degree: used to specify maximum bond degree between two atoms\n"
            << "   - specify by using -maxDeg n, where n is the desired maximum degree number\n"
            << "   - defaults to 4, if not specified\n\n"
            << "-unsaturations: used to specify the desired number of unsaturations. The following formats can be used:\n"
            << "   - -unsaturations n, in order to specify that there should be exactly n unsaturations\n"
            << "   - -unsaturations [n-m], in order to specify that there should be between n and m unsaturations\n"
            << "   - -unsaturations <n, or <=n, in order to specify that there should be at most n-1, or n, unsaturations\n"
            << "   - -unsaturations >n, or >=n, in order to specify that there should be at least n+1, or n, unsaturations\n"
            ...
            << "-single_bonds: used to specify the desired number of single bonds (incl. H-bonds)\n"
            << "   - same formats as for -unsaturations can be used\n\n"
            ...
            << "   - same formats as for -unsaturations can be used\n\n"
            << "-quadruple_bonds: used to specify the desired number of quadruple bonds\n"
            << "   - same formats as for -unsaturations can be used\n\n"
            << "-cycles-: used to specify the desired number of rings\n"
            << "   - same formats as for -unsaturations can be used\n\n"
            << "-substructure_files: used to specify the input file(s) in which the Substructures are defined. Several files can be listed in a row\n\n"
            << "-family_files: used to specify the input file(s) in which the Family Library is defined. Several files can be listed in a row\n\n"
            << "-element_alias_files: used to specify the input file(s) in which the elementAliases are defined. Several files can be listed in a row\n\n"
            << "-pseudoatom_files: used to specify the input file(s) in which the pseudoatoms are defined. Several files can be listed in a row\n\n"
            << "-families: used to specify the desired families, as defined in the Family Library file(s)\n"
            << "   - specified by giving the CODE of the family, as defined in the Family Library file(s)\n"
            << "   - several families can be listed after each other, or the -family keyword can be used several times in order to specify multiple familes\n\n"
            << "-stereo: used to specify that stereoisomerism should also be considered in the enumeration\n\n";
}

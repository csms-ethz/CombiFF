...
bool FindFragMatches(const enu::AdjacencyMatrix& A,
  ...
  if (!substructures.size()) return true;
  ...
  for (auto&& F : substructures) {
    ...
    if (F.GetAND()) {
      ...
      for (auto&& M : F.GetSubstructureMatrices()) {
        if (FindFragMatch(A, M, involved_atoms)) num_found++;
      }

      if (!IsInRange(num_found, F.GetRange())) return false;

    } else {
      for (auto&& M : F.GetSubstructureMatrices())
        num_matches.back() += FindFragMatch(A, M, involved_atoms);

      if (!IsInRange(num_matches.back(), F.GetRange())) return false;
    }
  }

  // for(size_t i = 0; i < substructures.size(); i++)
  // std::cout << "number of matches for " << FFragments.at(i).GetCode() << " is
  // "  << num_matches[i] << " (required range is " <<
  // *(FFragments.at(i).GetRange()) << " ) " <<'\n';
  return true;
}

int FindFragMatch(const enu::AdjacencyMatrix& A,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    for (size_t j = 0; j < m; j++) {
      if ((
              fragAtoms[i].GetElementSymbol() == atoms[j].GetElementSymbol() &&
              fragAtoms[i].GetNumConnections() <= atoms[j].GetNumConnections() /*fragAtoms[i].GetNumHydrogens() + fragAtoms[i].GetNumFixedHydrogens() == atoms[j].GetNumHydrogens() + atoms[j].GetNumFixedHydrogens()*/) ||
  ...
  UllmannMatch(M, matched_cols, matched_rows, k, n, m, A, fragment_matrix,
...
bool UllmannMatch(ComparisonMatrix& M, std::vector<bool>& matched_cols,
  ...
  if (k == (int)n - 1) {
    ...
    for (size_t j = 0; j < m; j++) {
      for (size_t i = 0; i < n; i++) {
        if (M.GetElement(i, j)) involved_atoms_local[j] = true;
      }
    }
  ...
  for (size_t l = 0; l < m; l++) {
    if (M.GetElement(k + 1, l) == true && matched_cols[l] == false &&
      ...
      if (Refine(M, k + 1, n, m, A, fragment_matrix)) {
        if (UllmannMatch(M, matched_cols, matched_rows, k + 1, n, m, A,
...
bool FindBenzMatch(enu::AdjacencyMatrix& A, bool& canonical,
  ...
  // AtomVector a = Atoms({"*", "*", "*", "*", "*", "*"});
  FragmentMatrix benzene(
      6,
      AdjacencyVector({0, 2, 0, 0, 0, 1, 2, 0, 1, 0, 0, 0, 0, 1, 0, 2, 0, 0,
                       0, 0, 2, 0, 1, 0, 0, 0, 0, 1, 0, 2, 1, 0, 0, 0, 2, 0}));
  const AtomVector<combi_ff::Atom>& atoms = A.GetAtomVector();
  ...
  UllmannMatchBenz(M, matched_cols, matched_rows, k, n, m, A, benzene_matrix,
                   found, num_matches, is_aromatic, canonical, u0);
  return found;
}

bool UllmannMatchBenz(ComparisonMatrix& M, std::vector<bool>& matched_cols,
                      std::vector<bool>& matched_rows, int k, const size_t n,
                      const size_t m, const enu::AdjacencyMatrix& A,
  ...
  if (k == (int)n - 1) {
    ...
    for (size_t i = 0; i < n; i++) {
      for (size_t j = 0; j < m; j++) {
        if (M.GetElement(i, j)) (is_aromatic_save)[j] = true;
      }
    }
    ...
    for (size_t i = 0; i < m; i++) {
      if ((is_aromatic_save)[i] == true) {
        ...
        if (num_aro_nbr > 2) num_with_2_plus_aromatic_neighbors++;
      }
    }
    ...
    for (size_t j = 0; j < m; j++) {
      for (size_t i = 0; i < n; i++) {
        if (M.GetElement(i, j)) in_benzene_ring[j] = true;
      }
    }
  ...
  for (size_t l = 0; l < m; l++) {
    if (M.GetElement(k + 1, l) == true && matched_cols[l] == false &&
      ...
      if (Refine(M, k + 1, n, m, A, fragment_matrix)) {
        if (canonical &&
            UllmannMatchBenz(M, matched_cols, matched_rows, k + 1, n, m, A,

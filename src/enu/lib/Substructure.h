...
class AbstractSubstructure {
 public:
  ...
  AbstractSubstructure(const size_t N, const AdjacencyVector& v,
...
typedef std::unordered_map<std::string, AbstractSubstructure>
    AbstractSubstructureMap;

class SubstructureCollection {
 private:
  std::vector<FragmentMatrix> substructure_matrices{
      std::vector<FragmentMatrix>(0)};
 ...
 public:
  ...
  SubstructureCollection(const AbstractSubstructure& F, const Range& r,
                         const AtomVector<combi_ff::Atom>& atoms,
                         const bool AND, const bool XOR);
  SubstructureCollection(const AbstractSubstructure& F, const Range& r,
                         const FragmentMatrix& M, const bool AND,
                         const bool XOR);
  SubstructureCollection(const AbstractSubstructure& F, const Range& r,

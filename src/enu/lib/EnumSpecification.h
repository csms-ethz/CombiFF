...
/*
EnumSpecifications:
- usedAtomVectors: vector that contains vectors with used atom types, e.g. for
C3H4Cl1 and C4Br1F2, usedAtomVectors = [[C,H,Cl], [C,Br,F]]
- lambdaRangesVec: for each AtomVector in usedAtomVectors, lambdaRangesVec
contains a vector of LambdaVectors that define the multiplicity of each of the
atoms
...
- rangedProperties: contains min and max value for the following properties:
                                         -rangedProperties[unsatRange] = number
of unsaturations -rangedProperties[bondRange] = number of bonds
                     -rangedProperties[SBrange] = number of single bonds (incl
H-bonds) -rangedProperties[DBrange] = number of double bonds

...
class Enumerator {
 public:
  Enumerator(const EnumSpecifications& enum_spec, const std::string& output_file_name) ;

  Enumerator(const Family& family, const PseudoatomMap& pseudoatoms, const bool stereo,
  ...
  void EnumerateFormula(const AtomVector<combi_ff::Atom>& atom_types, const LambdaVector& lambda,
                        const size_t max_degree, const bool stereo);

  void GetNImplicitAtoms(const AtomVector<combi_ff::Atom>& non_H_atoms, size_t& num_pseudoatoms_hyd);

  void GetFullAndExtendedMatrixSkeletons(const size_t N_full,
  ...
  void GetExtendedMatrix(RepresentationSystem*& u0, RepresentationSystem*& u_automorph,
                         RepresentationSystem*& u_id);

  void EnumerateStereoSmiles(const size_t n_rings, const RepresentationSystem& u_automorph,
                             const RepresentationSystem& u_id, const SmilesGeneratorEnu& smiles_gen);
  ...
  std::string CreateCanonicalFormula(const LambdaVector& l,
                                     const AtomVector<combi_ff::Atom>& used_atoms) ;
  /* void Enumerator::printMolecule(Enu::AdjacencyMatrix& A,const std::string& formula,const RepresentationSystem& u0,const RepresentationSystem& uAutomorph,const RepresentationSystem& id,const size_t nPSA,const Enu::AdjacencyMatrix& fullMatrixBU,const Enu::AdjacencyMatrix& extendedMatrixBU);*/
 ...
 private:
  ...
  // note: substructures and pseudoatoms are pointers s.t. we can use NULL for direct enumeration.
  // If direct enumeration is ever removed,these can be changed to references

...
Enumerator::Enumerator(const enu::EnumSpecifications& enum_spec,
                       const std::string& output_file_name)
    : code("mol"),
      lambda_ranges_vec(enum_spec.lambda_ranges_vec),
  ...
  *************************************************************************/
  for (size_t k = 0; k < used_atom_vectors.size(); k++) {
    const combi_ff::AtomVector<combi_ff::Atom>& atom_types =
        used_atom_vectors[k];
    const std::list<combi_ff::LambdaVector>& lambda_ranges =
        lambda_ranges_vec[k];
    ...
    for (const auto& lambda : lambda_ranges) {
      ...
      for (size_t i = 0; i < lambda.size(); i++) {
        degree_unsaturations_ +=
            ((double)atom_types[i].GetDegree() - 2.) * (double)lambda[i];
      ...
      ************************************************************/
      if (!(sum_degree % 2) &&
          IsInRange(deg_unsaturations,
                    ranged_properties[range_unsaturations])) {
        enumerator_arguments.formula_ptr =
            std::unique_ptr<const std::string>(new std::string(
                Enumerator::CreateCanonicalFormula(lambda, atom_types)));
        // print information on the new class of molecules,
        std::cout << "formula nr. " << ++formula_idx << ": "
                  << *enumerator_arguments.formula_ptr << '\n';
        // determine if the local maxDeg can be further restricted due to a
        // small DoU, as
        //  -> if DoU is zero, the max deg can be restricted to 1, if it's one
        //  the max deg can be restricted to 2 etc.
        //  -> thus, select the minimum of maxDeg and DoU + 1 for
        //  max_degree_local
        ...
        output_file << ">\n";
        Enumerator::EnumerateFormula(atom_types, lambda, max_degree_local,
                                     stereo);
        ...
        std::cout << " -> found " << num_isomers_for_formula
                  << " isomers for the formula "
                  << *enumerator_arguments.formula_ptr
                  << " with the given restrictions.\n"
                  << " -> total number of isomers in family so far is "
                  << num_isomers << "\n";
      }
    }
  }
...
void Enumerator::EnumerateFormula(const AtomVector<combi_ff::Atom>& atom_types,
                                  ...
                                  const size_t max_degree, const bool stereo) {
  ...
  if (!non_H_atoms.size()) return;

  // calculate total number of atoms N, number of non-hydrogen atoms Nhat, and
  // number of hydrogen atoms NH
  const size_t N(atoms.size());
  const size_t N_hat(non_H_atoms.size());
  const size_t N_hyd(N - N_hat);
  // define connectivity vector, which describes how the hydrogen atoms are
  // distributed among the non-hydrogen atoms i.e. if atom with index 2 is
  // connected to 3 hydrogen atoms, Hcon[2] is equal to 3
  enu::HydrogenDistribution hydrogen_dist(lambda, non_H_atoms, (int)N_hyd);
  // count the number of implicit hydrogen atoms and the number of implicit
  // atoms from pseudoatoms
  ...
  *************************************************************/
  ...
  while (hydrogen_dist.GetNextDistribution()) {
    ...
    // make local copy of non_H_atoms, where the implicit hydrogens are added to
    // the atoms
    combi_ff::AtomVector<combi_ff::Atom> atoms_hat(non_H_atoms);

    for (size_t i = 0; i < N_hat; i++)
      if (H_con[i]) atoms_hat[i].SetNumHydrogens(H_con[i]);

    // sort atomsHat and create the new lambda vector lambdaHat
    combi_ff::LambdaVector lambda_hat = Sort(atoms_hat);
    // prepare an atom vector that contains the fixed hydrogens from the united
    // atoms, in order to extend the found matrices in the next step
    enumerator_arguments.full_matrix_BU_ptr =
        std::unique_ptr<AdjacencyMatrix>(new AdjacencyMatrix(N_full));
    enumerator_arguments.extended_matrix_BU_ptr =
        std::unique_ptr<AdjacencyMatrix>(
    ...
    // define the adjacency matrix A, and the three supplementary matrices M, L,
    // C
    enumerator_arguments.A = std::shared_ptr<AdjacencyMatrix>(
        new AdjacencyMatrix(N_hat, lambda_hat, atoms_hat));
    enumerator_arguments.max_fill_alg_ptr = std::unique_ptr<MaxFillAlg>(
        new MaxFillAlg(*enumerator_arguments.A, stereo, max_degree));
...
void Enumerator::GetNImplicitAtoms(
  ...
  for (size_t k = 0; k < non_H_atoms.size(); k++) {
    ...
    } else if (non_H_atoms[k]
                   .GetNumFixedHydrogens())  // note: pseudoatoms can't have a
                                             // fixed NH, if this is needed,
                                             // another larger pseudoatom should
                                             // be created
    ...
    if (atoms_full[k].IsPseudoatom()) {
      ...
      for (size_t i = 1; i < psa->second.GetAtoms().size(); i++) {
        ...
        if (psa->second.GetAtoms()[i].GetNumFixedHydrogens()) {
          ...
          atoms_full.back().SetUnitedAtomSymbol(
              atoms_full.back().GetElementSymbol());
        }
      }
    ...
    for (size_t k = 0; k < atoms_hat.size(); k++) {
      if (atoms_hat[k].IsPseudoatom()) {
        ...
        for (size_t jj = 1; jj < psa->second.GetMatrix().GetN(); jj++) {
          enumerator_arguments.full_matrix_BU_ptr->SetElement(
              k, jj + pseudoatom_idx - 1,
              psa->second.GetMatrix().GetElement(0, jj));
          enumerator_arguments.extended_matrix_BU_ptr->SetElement(
              k, jj + pseudoatom_idx - 1,
              psa->second.GetMatrix().GetElement(0, jj));
        }
        ...
        for (size_t ii = 1; ii < psa->second.GetMatrix().GetN(); ii++) {
          for (size_t jj = ii + 1; jj < psa->second.GetMatrix().GetN(); jj++) {
            ...
            enumerator_arguments.full_matrix_BU_ptr->SetElement(
                ii + pseudoatom_idx - 1, jj + pseudoatom_idx - 1,
                psa->second.GetMatrix().GetElement(ii, jj));
            enumerator_arguments.extended_matrix_BU_ptr->SetElement(
                ii + pseudoatom_idx - 1, jj + pseudoatom_idx - 1,
                psa->second.GetMatrix().GetElement(ii, jj));
            //}
...
bool Enumerator::CheckMolecule() {
  ...
  // Get number of single, double, triple bonds, and number of rings in the
  // molecule
  std::fill(ranges.begin() + 1, ranges.end(), 0);
  enumerator_arguments.A->GetNumMultipleBonds(ranges);
  ranges[range_rings] = deg_unsaturations - (ranges[range_double_bonds] +
                                             2 * ranges[range_triple_bonds] +
                                             3 * ranges[range_quadruple_bonds]);

  // only accept the matrix, if the bond/ring restrictions are fulfilled
  if (AreInRange(ranges, ranged_properties) &&
      (substructures == NULL ||
...
/*
generate the SMILES (and stereo SMILES if specified) and write them to the
output_file
*/
...
void Enumerator::PrintConstitutionalIsomer(const std::string& smiles) {
  output_file << std::string(xml_indent_size * 3, ' ') << "<isomer isomer_id=\""
              ...
              << "\">\n"
              /* << std::string(xml_indent_size * 3, ' ') << "<formula>" <<
               *enumerator_arguments.formula << "</formula>\n"*/
...
void Enumerator::GetFullMatrix() {
  ...
  for (size_t i = 0; i < N; i++) {
    for (size_t j = i + 1; j < N; j++)
      enumerator_arguments.full_matrix_ptr->SetElement(
          i, j, enumerator_arguments.A->GetElement(i, j));

    size_t deg = enumerator_arguments.full_matrix_ptr->AccumulateRow(i);

    for (size_t j = deg + 1;
         j <=
         enumerator_arguments.full_matrix_ptr->GetAtomVector()[i].GetDegree();
         j++)
  ...
  for (size_t i = N; i < N + enumerator_arguments.n_pseudoatoms; i++) {
    ...
    for (size_t j = deg + 1;
         j <= enumerator_arguments.full_matrix_ptr->GetAtomVector()[i]
                  .GetDegree() /*A.GetAtomVector()[i].GetNumHydrogens()*/;
         j++)
      enumerator_arguments.full_matrix_ptr->SetElement(i, H_index++, 1);
  }
}
...
void Enumerator::GetExtendedMatrix(RepresentationSystem*& u0,
  ...
  for (size_t i = 0; i < N; i++) {
    for (size_t j = i + 1; j < N; j++)
      enumerator_arguments.extended_matrix_ptr->SetElement(
          i, j, enumerator_arguments.A->GetElement(i, j));
  }
  ...
  if (stereo) {
    ...
    while (perm_it.GetNextPermutation()) {
      ...
      if (enumerator_arguments.extended_matrix_ptr->EqualTo(
              *(perm_it.GetPermutedIndices()), jpos)) {
...
void Enumerator::EnumerateStereoSmiles(const size_t n_rings,
  ...
  std::vector<std::tuple<std::string, int, std::pair<int, int>>>
      stereo_smiles_vec = stereo_gen.GetStereoSmiles();
  ...
  if (stereo_smiles_vec.size()) {
    ...
    for (size_t n = 0; n < stereo_smiles_vec.size(); n++) {
      ...
      output_file << std::string(xml_indent_size * 5, ' ')
                  << "<stereo_isomer stereo_id=\"" << code << "_"
                  << std::setw(9) << std::setfill('0') << std::right
                  << std::to_string(num_isomers_for_listing) << "_"
                  << std::setw(4) << std::to_string(n + 1) << "\">\n"
                  << std::string(xml_indent_size * 6, ' ') << "<stereo_SMILES>"
      ...
      if (enantiomer != -1)
        output_file << std::string(xml_indent_size * 6, ' ') << "<enantiomer>"
                    << code << "_" << std::setw(9) << std::setfill('0')
                    << std::right << std::to_string(num_isomers_for_listing)
                    << "_" << std::setw(4) << std::to_string(enantiomer + 1)
                    << "</enantiomer>\n";

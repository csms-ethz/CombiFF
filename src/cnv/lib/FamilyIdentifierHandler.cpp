  ...
  for (auto fmi_it = input_list.begin(); fmi_it != input_list.end(); ++fmi_it) {
    for (size_t i = 0; i < fmi_it->second.size(); i++) {
      if ((fmi_it->second)[i] == ' ' || (fmi_it->second)[i] == ',') {
        ...
        input_list.insert(
            next, {fmi_it->first,
                   fmi_it->second.substr(i + 1, fmi_it->second.size() - i)});
...
void FamilyIdentifierHandler::ConvertFamilyIdentifier(const std::string& fmi) {
  ...
  if (!found) {
    std::cout << "!Error: no family with the code " << family_code
              << " for compound " << fmi
              << " found in any of the family isomer enumeration files ";
...
void FamilyIdentifierHandler::SearchFileForIsomer(
    const std::string& fmi, const std::string& family_file_name) {
  std::ifstream family_file(family_file_name);

  if (!family_file.is_open()) {
    std::cout << "!Error: couldn't open family file " << family_file_name
              << " for compound " << fmi << "\n";
    exit(-1);
  }
  ...
  if (!found) {
    std::cout << "!Error: couldn't find compound " << fmi << " in "
              << family_file_name << "\n";
    exit(-1);
  }
    ...
    if (print_options[cnv::print_mass] || print_options[cnv::print_stack] ||
        print_options[cnv::print_matrix] ||
...
void FamilyIdentifierHandler::PrintOutput(const std::string& fmi,

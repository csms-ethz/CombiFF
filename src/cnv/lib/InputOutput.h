...
typedef enum {smiles, formula, name, family_enumeration, matrix, not_set} InputOption;
typedef enum {print_canon_smiles,
              print_canon_name,
              print_canon_formula,
              print_mass,
...
class InputOutput {
 public:
  ...
  void ReadInputOption(const size_t pos, const combi_ff::StringVector& arguments) ;
  void ReadOutputOption(const size_t pos, const combi_ff::StringVector& arguments) ;
  void ReadFieFileNamesFromSetupFile(const std::string& setup_file_name);
  const combi_ff::StringVector& GetInputFileNames() const;
  const std::string IncompatibilityMessage(const std::string& input, const std::string& output) const;

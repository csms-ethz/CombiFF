...
class Handler {
 protected:
  ...
  std::list<std::pair<std::string, std::string>> input_list; //pair: (input source, input)


 public:

  Handler(std::ofstream& output_file, StringVector& fie_file_names, const int column_width, std::vector<bool>& print_options,
          std::list<std::pair<std::string, std::string>>& input_list):
    output_file(output_file), fie_file_names(fie_file_names), column_width(column_width), print_options(print_options), input_list(input_list) {}

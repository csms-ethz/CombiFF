...
struct SmilesBlock {
  ...
  // ring_indices: first element is bond degree, 2nd element is ring index (e.g. C=1 -> ("=",1))
...
class SmilesGenerator {
 public:
  ...
  void Refine();
  void SortRingNodes(std::vector<std::tuple<size_t, size_t, size_t>>& ring_nodes,
...
template<typename T, typename AtomClass>
SmilesGenerator<T, AtomClass>::SmilesGenerator(const
                                               combi_ff::AdjacencyMatrix<T, AtomClass>& A) :
  A(A), N(A.GetN()), is_aromatic_carbon(A.GetIsAromaticCarbonConst()),
  ...
  ring_connections(std::vector<std::vector<size_t>>(N, std::vector<size_t>(0))),
  smiles(std::string("")) {
...
void SmilesGenerator<T, AtomClass>::GenerateSmiles() {
  ...
  for (size_t i = 0; i < N; i++)
    indices[i] = partition_classes[i].front();
...
std::string SmilesGenerator<T, AtomClass>::CreateSmiles() {
  ...
  size_t nArom(0);
  //indices: indices[i] = j means that atom j has priority i, aka that at priority i is atom j
  ...
  for (size_t i = 0; i < indices.size(); i++) {
    ...
    if (is_aromatic_carbon[indices[i]]) {
      ...
      std::transform(smiles_blocks[indices[i]].element_name.begin(),
                     ...
                     smiles_blocks[indices[i]].element_name.begin(),
                     tolower);
    ...
    if (A.GetAtomVector()[indices[i]].GetHydrogenInSmiles()) {
      if (A.GetAtomVector()[indices[i]].GetNumTotalHydrogens()) {
        ...
        smiles_blocks[indices[i]].element_name += "H" + std::to_string(nH) + "]";
  ...
  //std::vector<std::vector<size_t>> going_to(indices.size(), std::vector<size_t>(0));
  ...
  while (!stack.empty()) {
    ...
    if (!written[i]) {
      ...
      if (i != indices[0])
        going_to[coming_from[i]].push_back(i);
      ...
      for (int j = (int)indices.size() - 1; j >= 0; j--) {
        if (A.GetElement(i, indices[j])) {
          ...
          if (!written[indices[j]]) {
            ...
            if (A.GetElement(i, indices[j]) == 2 && !(is_aromatic_carbon[indices[j]] && is_aromatic_carbon[i]))
            ...
            //std::cout << " . coming from at " << indices[j] << " is " << i << std::endl;
            ...
            if (ring > 99)
              throw std::runtime_error("can't handle more than 99 rings in a SMILES string");

            ring_nodes.push_back(std::tuple<size_t, size_t, size_t>(indices[j], i, ring));
            ...
            if (A.GetElement(i, indices[j]) == 1 ||
              ...
              name.ring_indices.push_back(std::pair<std::string, size_t>("", ring));
              name.bond_type = "";
              ...
              smiles_blocks[indices[j]].ring_indices.push_back(
                std::pair<std::string, size_t>("#", ring));// += '#' + std::to_string(ring);
              name.bond_type = "";
            ...
            if (A.GetElement(coming_from[i], i) == 2 && (!is_aromatic_carbon[i] ||
                                                         !is_aromatic_carbon[coming_from[i]]))
              name.bond_type = "=";
...
//      for(size_t ii = 0; ii < A.GetAtomVector()[v].GetNumTotalHydrogens(); ii++){
//        smiles += "(H)";
//      }
    //end hack
...
template<typename T, typename AtomClass>
void SmilesGenerator<T, AtomClass>::Tiebreak(std::vector<size_t>&
                                             curr_partition) {
...
void SmilesGenerator<T, AtomClass>::Refine() {
  ...
  for (size_t i = 0; i < N; i++) {
    if (partition_classes[i].size() > 1)
      partitions_to_sort.push_back(i);
  }
  ...
  while (!partitions_to_sort.empty()) {
    ...
    //sort the current partitions and store which ones were changed in changed_indices
    Sort(curr_partition);
    ...
    std::sort(affected_partition_classes.begin(), affected_partition_classes.end());
    auto&& it = std::unique(affected_partition_classes.begin(),
                            affected_partition_classes.end());
    affected_partition_classes.resize(std::distance(
                                        affected_partition_classes.begin(),
                                        it));
...
void SmilesGenerator<T, AtomClass>::TestValidity(const std::string& smiles,
                                                 ...
                                                 const size_t& ring) {
  {
    ...
    for (auto && v : visited_indices) {
      if (smiles_blocks[v].opening_braces != "")
        braceOpen.push(1);
    ...
    if(nD != nDouble || nT != nTriple || nQ != nQuadruple || ring != nRings) {
        std::cout << "\nwrong number of double/triple/quadruple bonds in SMILES: " << smiles << "\n"

...
PermutationIterator::PermutationIterator(const combi_ff::RepresentationSystem& u)
...
const std::vector<size_t>* PermutationIterator::GetPermutedIndices(size_t& highest_permuted_index) {
  std::iota(permuted_indices.begin(), permuted_indices.end(), 0);
  size_t hi = 0;
...
bool PermutationIterator::GetNextPermutation() {
  while (current_index < p.size()) { //note: current_index is unsigned -> when current_index = -1, modular arithmetics leads to current_index > p.size
...
const Permutations* PermutationIterator::GetCombinedPermutation() {
  ...
  for (size_t ii = smallest_diff_index; ii < p.size(); ii++) {
    if (p[ii]) {
      permutations.insert(permutations.end(), (*u)[ii][p[ii]].begin(), (*u)[ii][p[ii]].end());
    }
  }
...
size_t PermutationIterator::GetCurrentIndex() const {
...
void PermutationIterator::Reset(const combi_ff::RepresentationSystem* u_, const size_t current_new) {
  current_index = current_new;
  smallest_diff_index = current_index;
  std::fill(p.begin(), p.end(), 0);
  u = u_;

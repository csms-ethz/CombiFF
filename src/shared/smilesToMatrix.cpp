...
combi_ff::AdjacencyMatrix<double, combi_ff::CnvAtom> ConvertToMatrix(
  ...
  if (smiles.size() == 1 && !isalpha(smiles[0]))
    throw std::runtime_error(
        "can't convert a smiles string that only consists of one symbol and "
        "doesn't contain an atom: " +
        smiles);
...
bool GetBasicInformationFromSmiles(
  ...
  if (n_unsaturations - 1 + (int)atoms.size() - 0.5 * (double)degree > 0)
    std::cerr <<
              "?Warning: nUnsat larger than zero in smilesToMatrix
  transformation of " << smiles << " (unsat= " << n_unsaturations << " #atoms =
...
void CreateNameVector(std::vector<combi_ff::SmilesBlock>& names,
  ...
  for (size_t i = 0; i < smiles.size(); i++) {
    if (smiles[i] == '=' && smiles[i + 1] != '%' &&
      ...
      if (curr_atom >= names.size())
        throw combi_ff::input_warning(
            "encountered unexpected symbol " + std::string(1, smiles[i]) +
            " at position " + std::to_string(i) + " in " + smiles);
      ...
      if (smiles[i] == '@') {
        ...
        atoms[curr_atom - 1].SetFormalCharge(
            atoms[curr_atom - 1].GetFormalCharge() + smiles[i]);
        i++;
      ...
      if (smiles[i] == 'H') {
        ...
        if (names[curr_atom - 1].element_name.front() == '[') {
          ...
          if (std::isdigit(smiles[i])) {
            ...
            if (++i == smiles.size())
              throw combi_ff::input_warning("unclosed [ encountered in " +
                                            smiles);
          }

          names[curr_atom - 1].element_name += "H" + std::to_string(num_h);
          atoms[curr_atom - 1].SetDegree(atoms[curr_atom - 1].GetDegree() +
                                         num_h);
        ...
        if (smiles[i] != ']')
          throw combi_ff::input_warning(
              "encountered " + std::string(1, smiles[i]) + " at position " +
              std::to_string(i) + " in " + smiles + ". but expected ]");
        ...
        throw combi_ff::input_warning(
            ...
            " at position " + std::to_string(i) + " in " + smiles);
               ...
               (isdigit(smiles[i + 1]) || smiles[i + 1] == '%')) {
      // nothing is to be done, this is just s.t. there is no error recognized
      // in the final else clause! DO NOT REMOVE
    } else if (smiles[i] == '-') {
      ...
      throw std::runtime_error("encountered unexpected symbol " +
                               std::string(1, smiles[i]) + " at position " +
                               std::to_string(i) + " in " + smiles);
  }
  ...
  // check if reconstructed smiles is equal to original one (ignore explicit
  // single bonds)
    ...
    if (names[ii].closing_braces.size() > 1) {
      for (size_t jj = 1; jj < names[ii].closing_braces.size(); jj++) {
        names[braceStack.top()].opening_braces =
            names[braceStack.top()].opening_braces.substr(
                0, names[braceStack.top()].opening_braces.size() - 1);
        braceStack.pop();
      }
...
void CreateMatrixFromSmilesBlocks(
    ...
    const size_t nR) {
  std::vector<std::pair<int, int>> ring_indices(nR,
  ...
  for (size_t i = 1; i < names.size(); i++) {
    ...
    if (name.opening_braces == "" && names[i - 1].closing_braces == "") {
      if (A.GetElement(i - 1, i))
        throw std::runtime_error("already Set value of " + std::to_string(i) +
                                 " and " + std::to_string(i - 1));

      if ((std::islower(name.element_name.front()) ||
           (name.element_name.front() == '[' &&
            std::islower(name.element_name[1]))) &&
      ...
      // encountered the final branch of an atom (because previous closing
      // braces are ')')
      if (!atmIdx.size())
        ...
        if (ring_indices[r].first != -1) {
          if (std::islower(names[ring_indices[r].first].element_name.front()) &&
            ...
            A.SetElement(ring_indices[r].first, i, 1.5);
                   ...
                   names[ring_indices[r].first]
                           .ring_indices[ring_indices[r].second]
                           .first[0] == '=')
  ...
  if (names.back().closing_braces != "") atmIdx.pop();
  ...
  for (size_t i = 0; i < A.GetAtomVector().size(); i++) {
    ...
    if (int(rowSum * 10) % 10) {
      ...
      //           i) << " in " << smiles << " is not an integer (ok for fused
      //           aromatic rings)\n";
    }
    ...
    if (rowSum > (double)A.GetAtomVector()[i].GetDegree() &&
      ...
      //                        rowSum); // e.g. for S, where different valences
      //                        are possible
    }

    if ((double)A.GetAtomVector()[i].GetDegree() > rowSum && !A.GetAtom(i)
                                                                  .GetHydrogenInSmiles() /*&& A.GetAtomVector()[i].GetElementSymbol() == "C"*/)  // add implicit hydrogens
      (A.GetAtom(i))
          .SetNumHydrogens(

...
XmlElement::XmlElement(XmlElement* parent) : parent(parent), distance(parent->distance + 1), file_name(parent->file_name) {}
XmlElement::XmlElement(XmlElement* parent, const std::string& tag) : parent(parent), distance(parent->distance + 1), file_name(parent->file_name),
	tag(tag) {}
XmlElement::XmlElement(XmlElement* parent, const XmlElement& element) : parent(parent), distance(parent->distance + 1),  file_name(parent->file_name),
	tag(element.tag),
	attributes(element.attributes), value(element.value) {}
...
void XmlElement::CheckNumberOfChildren_equal(const size_t n) const {
	if(children.size() != n)
		throw wrong_amount_of_children_error(tag, std::to_string(n), children.size());
}
...
void XmlElement::CheckNoValue() const {
	if(value.size())
		throw unexpected_value_error(tag, value);
}
void XmlElement::CheckValue() const {
	if(!value.size())
		throw missing_value_error(tag);
}

void XmlElement::SetAttribute(const XmlElement& el, const std::string& attributeName) {
...
XmlElement_ptr XmlElement::GetLastChild() const {
	return children.back();
}

XmlElement_ptr XmlElement::GetFirstChild() const {
...
const size_t XmlElement::GetNumberOfChildren() const {
...
XmlTree::XmlTree(const std::string& roottag) : root() {
	root.tag = roottag;
}
XmlElement& XmlTree::GetRoot() {
	return root;
}
const XmlElement& XmlTree::GetRoot_const() const {
	return root;
}
XmlElement* XmlTree::GetRootPointer() {
	return &root;
}
void XmlTree::SetRoot(const std::string& root_tag, std::map<std::string, std::string>& attributes) {
	root.tag = root_tag;
	root.attributes = attributes;
}
void XmlTree::CheckRootTagName(const std::string& root_tag) const {
	if(root.tag != root_tag)
		throw wrong_root_tag_error(root_tag, root.tag);
}
...
XmlParser::XmlParser(const std::string& file_name, const std::string& root_tag_) : tree(root_tag_), file_name(file_name), root_tag(root_tag_),
	dtd(root_tag + ".dtd") {
	tree.GetRoot().SetFileName(file_name);
}
...
const XmlTree& XmlParser::GetTree_const() const {
	return tree;
}
const std::string& XmlParser::GetVersion() const {
	return version;
}
const std::string& XmlParser::GetEncoding() const {
	return encoding;
}
const std::string& XmlParser::GetRootTag() const {
	return root_tag;
}
const std::string& XmlParser::GetDtd() const {
	return dtd;
}
...
XmlParserIn::XmlParserIn(const std::string& file_name, const xml_read_mode mode) : XmlParser(file_name), xml_file(file_name), read_mode(mode) {
	if(!xml_file.is_open())
...
void XmlParserIn::ReadDocInfo() {
	...
	if(tag_content.size() < 2 || tag_content[1] != '!') { //we know that size of tag_content is at least 3 (checking in readNextTag)
	...
	if(root_tag.back() == '>') {
		...
		if((opening_quote != '\'' && opening_quote != '\"') || opening_quote != closing_quote)
			throw mismatched_quotes_error(file_name, "tag_content", dtd, opening_quote, closing_quote);
		...
		while(tag_content != "</" + tree.GetRoot().tag + ">") {
			...
			if(tag_content.size() > 1 && tag_content[1] == '/') {
				if(tag_content.substr(2, cur_node->tag.size()) != cur_node->tag)
					throw unexpected_closing_tag_error(file_name, tag_content, "</" + cur_node->tag + ">");
				...
				if(tag_content != ">") {
					std::map<std::string, std::string> attributes = ReadAttributes(elementName, tag_content);
...
void XmlParserIn::CheckRootTag() {
	...
	if(tag_content == "/>")
		std::cout << "only root tag found\n";
}

std::map<std::string, std::string> XmlParserIn::ReadAttributes(const std::string& tag, std::string& tag_content) {
	std::map<std::string, std::string> attributes;
	...
	while(tag_content != ">" && tag_content != "/>") {
		...
		if(attributes.find(attribute_type) != attributes.end())
			throw duplicate_attributes_error(file_name, tag, attribute_type, attribute_value, attributes[attribute_type]);
...
std::string XmlParserIn::ReadNextTag() {
	...
	if(ch != '<') {
		...
		throw opening_tag_error(file_name, std::string(1, ch) + tag_content, std::string(1, ch) + tag_content);
	}
		...
		while(!xml_file.eof() && xml_file.peek() != '>') {
			...
			if(ch == ' ' || ch == '\t') {
				while(!xml_file.eof() && (xml_file.peek() == ' ' || xml_file.peek() == '\t'))
					xml_file.get();
			}
		}
	...
	if(tag_content.substr(0, 4) == "<!--") {
		while(!xml_file.eof() && tag_content.substr(tag_content.size() - 3, 3) != "-->") {
			while(xml_file.peek() != '>') {
				xml_file >> ch;
				tag_content += ch;
			}
...
std::string XmlParserIn::ReadNextWord(std::string& tag_content) {
	...
	while(tag_content[i] == ' ' || tag_content[i] == '\t') //not really necessary, because readNextTag removes duplicate whitespaces
		i++;
...
std::string XmlParserIn::ReadElementName(std::string& tag_content) {
	...
	if(element_name == "<") //case that there's a whitespace between '<' and element name
...
std::string XmlParserIn::ReadAttribute(const std::string& attribute_keyword, std::string& tag_content, const TagType type) {
	std::string word = ReadNextWord(tag_content);

	if(word.size() < attribute_keyword.size() || word.substr(0, attribute_keyword.size()) != attribute_keyword)
		throw unexpected_word_error(file_name, tag_content, attribute_keyword + "= (attribute)", word);
	...
	if((opening_quote != '\"' && opening_quote != '\'') || closing_quote != opening_quote)
		throw mismatched_quotes_error(file_name, tag_content, word, opening_quote, closing_quote);
...
std::string XmlParserIn::ReadAttributeType(std::string& tag_content) {
	...
	if(pos == std::string::npos) {
		...
		if(word.front() != '=')
			throw unexpected_word_error(file_name, tag_content, "=", word + " " + tag_content);
		...
		tag_content = attribute_type.substr(pos + 1, attribute_type.size()) + " " + tag_content;
...
std::string XmlParserIn::ReadAttributeValue(std::string& tag_content) {
	...
	if((opening_quote != '\'' && opening_quote != '\"') || opening_quote != closing_quote)
		throw mismatched_quotes_error(file_name, tag_content, attribute_value, opening_quote, closing_quote);
...
void XmlParserIn::ReadXMLInfo() {
	...
	if(tag_content[1] != '?') { //we know that size of tag_content is at least 3 (checking in readNextTag)
		std::cout << "?Warning: didn't find any info on version (using default version " << version
		          << ") or encoding (using default encoding " << encoding << ")\n";
		xml_file.seekg(0);
		return;
	}
	...
	std::string version_string = ReadAttribute("version", tag_content, question_mark);
	...
	if(tag_content == "?>") {
		std::cout << "?Warning: didn't find encoding info, using default " << encoding << "\n";
		return;
	}
	...
	if(!xml_file.eof()) {
		...
		while(tag_content != "</" + read_until_tag_name + ">") {
			...
			if(tag_content.size() > 1 && tag_content[1] == '/') {
				...
				cur_node = cur_node->parent;
				tag_content = ReadNextTag();
				//				std::cout << "    a " << tag_content << std::endl;
				...
				if(tag_content != ">") {
					std::map<std::string, std::string> attributes = ReadAttributes(element_name, tag_content);
					cur_node->attributes = attributes;
				}
...
void XmlParserIn::RemoveChildrenOfCurrentNode() {
	cur_node->RemoveChildren();
}

bool XmlParserIn::eof() const{
	return xml_file.eof();
}
...
XmlParserOut::XmlParserOut(const std::string& file_name, const std::string& root_tag) : XmlParser(file_name, root_tag), xml_file(file_name) {
	if(!xml_file.is_open())
...
std::ostream& operator<<(std::ostream& stream, const XmlParser& parser) {
	stream << "<?xml version='" << parser.GetVersion() << "' encoding='" << parser.GetEncoding() << "'?>\n";
	stream << "<!DOCTYPE " << parser.GetRootTag() << " SYSTEM \"" << parser.GetDtd() << "\">\n";

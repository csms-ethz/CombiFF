...
struct XmlElement: std::enable_shared_from_this<XmlElement> {
	...
	void AddElement(const std::string& tag, const std::list<XmlElement_ptr>::iterator pos) ;
...
class XmlTree {
  public:
	XmlTree() = default;
	XmlTree(const std::string& root_tag) ;
	XmlElement& GetRoot();
	const XmlElement& GetRoot_const() const ;
	XmlElement* GetRootPointer() ;
	void SetRoot(const std::string& root_tag_, std::map<std::string, std::string>& attributes) ;
...
class XmlParserIn : public XmlParser {
  public:
	...
	void BuildTree();
	void CheckRootTag();
	std::map<std::string, std::string> ReadAttributes(const std::string& tag, std::string& tag_content) ;
	std::string ReadNextTag();
	std::string ReadNextWord(std::string& tag_content);
	std::string ReadAttribute(const std::string& attribute_keyword, std::string& tag_content, const TagType type);
